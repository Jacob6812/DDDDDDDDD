...
class FinnhubClient:
    ...
    def __init__(
        ...
        """
        self.api_key = api_key or os.getenv("FINNUB_API_KEY", "")
        ...
        if not self.api_key:
            ...
            try:
                ...
                logger.info(
                    f"Finnhub client initialized successfully with timeout: {timeout}s"
                )
    ...
    def get_company_news(
        ...
        """
        logger.debug(
            f"🔍 [Finnhub] API request parameters: symbol={symbol}, start_date={start_date}, end_date={end_date}, limit={limit}"
        )

        if not self.is_available():
            logger.error(
                "❌ [Finnhub] Client unavailable - check API key or network connection"
            )
            return []
        ...
        try:
            ...
            logger.debug(
                f"🌐 [Finnhub] Starting API call: company_news({symbol}, {start_date}, {end_date}) with timeout: {news_timeout}s"
            )
            ...
            logger.debug(
                f"⏱️ [Finnhub] API call completed, time taken: {elapsed_time:.2f}s"
            )

            if not news_data:
                logger.warning(
                    f"⚠️ [Finnhub] API returned empty result - symbol: {symbol}, time range: {start_date} to {end_date}"
                )
                return []
            ...
            if news_data:
                ...
                if timestamps:
                    ...
                    logger.debug(
                        f"📅 [Finnhub] News timestamp examples: {', '.join(times_str)}..."
                    )
                else:
                ...
                headlines_count = sum(
                    1 for item in news_data if item.get("headline", "").strip()
                )
                summaries_count = sum(
                    1 for item in news_data if item.get("summary", "").strip()
                )
                logger.debug(
                    f"📝 [Finnhub] Data integrity: headlines {headlines_count}/{len(news_data)}, summaries {summaries_count}/{len(news_data)}"
                )
            ...
            if len(news_data) > limit:
                ...
                logger.debug(
                    f"✂️ [Finnhub] Applied quantity limit: {len(news_data)}/{original_count} (limit: {limit})"
                )
            ...
            for idx, item in enumerate(news_data):
                try:
                    ...
                    if datetime_ts and datetime_ts > 0:
                        ...
                        if (
                            idx < 3
                        ):  # Only log warnings for the first few to avoid excessive logging
                            logger.debug(
                                f"⚠️ [Finnhub] News item {idx + 1} has no valid timestamp"
                            )
                    ...
                    standardized_item = {
                        ...
                        "related_symbols": [
                            symbol
                        ],  # Finnhub company news default associated with queried stock
                    ...
                    if conversion_errors <= 3:  # Only log the first few errors
                        logger.warning(
                            f"⚠️ [Finnhub] News item {idx + 1} format conversion failed: {convert_exc}"
                        )

            if conversion_errors > 0:
                logger.warning(
                    f"⚠️ [Finnhub] Total {conversion_errors} news items failed format conversion"
                )
            ...
            if standardized_news:
                valid_items = sum(
                    1
                    for item in standardized_news
                    if item.get("title") or item.get("description")
                )
                logger.info(
                    f"✅ [Finnhub] Successfully retrieved {symbol} news: {len(standardized_news)} items (valid content: {valid_items})"
                )
            ...
            logger.error(
                f"❌ [Finnhub] Failed to retrieve {symbol} news: {e}", exc_info=True
            )
            ...
            if any(
                ...
                if "api" in error_str or "key" in error_str:
                    logger.debug(
                        f"🔍 [Finnhub] API key related issue: check API key validity"
                    )
                    raise FinnhubError(
                        f"API key error: {e}", error_code="API_KEY_ERROR"
                    )
                elif (
                    ...
                    logger.debug(
                        f"🔍 [Finnhub] Permission issue: check API key validity and account permissions"
                    )
                    raise FinnhubError(
                        f"Permission denied: {e}", error_code="PERMISSION_ERROR"
                    )
                elif "rate limit" in error_str:
                    logger.debug(
                        f"🔍 [Finnhub] Rate limit: suggest reducing request frequency"
                    )
                    raise FinnhubError(
                        f"Request rate too high: {e}", error_code="RATE_LIMIT_ERROR"
                    )
                else:
            ...
            # For temporary errors like network timeout, log but return empty list
                ...
                logger.debug(
                    f"🔍 [Finnhub] Timeout issue: check network connection stability"
                )
                return []
            else:
    ...
    def get_company_profile(self, symbol: str) -> Dict:
        ...
        try:
            ...
            if profile:
                logger.debug(
                    f"✅ [Finnhub] Successfully retrieved profile for {symbol}"
                )
                return profile
            else:
            ...
            logger.error(
                f"❌ [Finnhub] Failed to get company profile for {symbol}: {e}"
            )
            return {}
    ...
    def get_basic_financials(self, symbol: str, metric: str = "all") -> Dict:
        ...
        try:
            logger.debug(
                f"📊 [Finnhub] Getting basic financials for {symbol}, metric={metric}"
            )
            financials = self.client.company_basic_financials(
                symbol=symbol, metric=metric
            )

            if financials:
                logger.debug(
                    f"✅ [Finnhub] Successfully retrieved financials for {symbol}"
                )
                return financials
            else:
            ...
            logger.error(
                f"❌ [Finnhub] Failed to get basic financials for {symbol}: {e}"
            )
            return {}
    ...
    def get_stock_indicators(
        ...
        if not self.is_available():
            logger.warning(
                f"❌ [FUNDAMENTAL_DATA] Finnhub client unavailable for {symbol}"
            )
            return result
        ...
        try:
            ...
            logger.info(
                f"🏢 [FUNDAMENTAL_DATA] Step 1: Getting company profile for {symbol}"
            )
            ...
            if profile and "marketCapitalization" in profile:
                ...
                if market_cap_millions:
                    ...
                    logger.info(
                        f"✅ [FUNDAMENTAL_DATA] Market cap: ${result['market_cap']:,.0f}"
                    )
            else:
            ...
            logger.info(
                f"📊 [FUNDAMENTAL_DATA] Step 2: Getting basic financials for {symbol}"
            )
            ...
            if financials and "metric" in financials:
                ...
                if pe_ttm and pe_ttm > 0:
                    ...
                    logger.info(
                        f"✅ [FUNDAMENTAL_DATA] P/E ratio: {result['pe_ratio']:.2f}"
                    )
                ...
                if dividend_yield and dividend_yield > 0:
                    ...
                    logger.info(
                        f"✅ [FUNDAMENTAL_DATA] Dividend yield: {result['dividend_yield']:.2f}%"
                    )
                ...
                if week_52_high and week_52_high > 0:
                    ...
                    logger.info(
                        f"✅ [FUNDAMENTAL_DATA] 52-week high: ${result['week_52_high']:.2f}"
                    )
                if week_52_low and week_52_low > 0:
                    result["week_52_low"] = float(week_52_low)
                    logger.info(
                        f"✅ [FUNDAMENTAL_DATA] 52-week low: ${result['week_52_low']:.2f}"
                    )
            else:
                logger.warning(
                    f"⚠️ [FUNDAMENTAL_DATA] No financial metrics for {symbol}"
                )

            # 3. Set default dividend data (Finnhub API limitation)
            logger.info(
                f"💰 [FUNDAMENTAL_DATA] Step 3: Setting default dividend data for {symbol}"
            )
            ...
            if "quarterly_dividend" not in result:
                ...
                logger.info(
                    f"ℹ️ [FUNDAMENTAL_DATA] Using default quarterly dividend: $0.0000"
                )

            logger.info(
                f"🎯 [FUNDAMENTAL_DATA] Finnhub indicators summary for {symbol}:"
            )
            ...
            logger.error(
                f"❌ [FUNDAMENTAL_DATA] Failed to get stock indicators for {symbol}: {e}"
            )
        ...
        try:
            ...
            for item in news_data:
                standardized_item = {
                    ...
                    "category": item.get("category", ""),
                    "image": item.get("image", ""),
                    "related_symbols": item.get(
                        "related", []
                    ),  # Finnhub general news may contain related stocks
            ...
            logger.info(
                f"Finnhub successfully retrieved general news: {len(standardized_news)} items"
            )
            return standardized_news
            ...
            error_str = str(e).lower()
            logger.error(
                f"❌ [Finnhub] Failed to retrieve general news: {e}", exc_info=True
            )
    ...
    def get_stock_indicators(
        ...
        try:
            ...
            logger.info(
                f"💰 [FUNDAMENTAL_DATA] Step 3: Getting dividend data for {symbol} from Polygon API"
            )
            try:
                ...
                if not dividends_df.empty:
                    ...
                    if "ex_dividend_date" in dividends_df.columns:
                        dividends_df = dividends_df.sort_values(
                    ...
                    if latest_dividend and latest_dividend > 0:
                        ...
                        logger.info(
                            f"✅ [FUNDAMENTAL_DATA] Latest quarterly dividend from Polygon: ${result['quarterly_dividend']:.4f}"
                        )
                    else:
                        logger.info(
                            f"ℹ️ [FUNDAMENTAL_DATA] No valid dividend amounts for {symbol}"
                        )
                else:
                    logger.info(
                        f"ℹ️ [FUNDAMENTAL_DATA] No dividend data from Polygon for {symbol}"
                    )
            except Exception as e:
                logger.warning(
                    f"⚠️ [FUNDAMENTAL_DATA] Failed to get dividend data from Polygon for {symbol}: {e}"
                )
                logger.info(
                    f"ℹ️ [FUNDAMENTAL_DATA] Using default dividend value for {symbol}"
                )
