...
class PolygonClient:
    ...
    def _request(self, method: str, path: str, params: Optional[Dict] = None) -> Dict:
        ...
        # Retry policy: honour env-var overrides so tradingagents_baseline
        # and other callers can enforce a 100 attempt × 3 s fixed-interval
        # floor. Falls back to exponential backoff when env vars are unset.
        try:
            _max_attempts = int(os.getenv("POLYGON_CLIENT_MAX_ATTEMPTS") or 6)
        except (TypeError, ValueError):
            _max_attempts = 6
        try:
            _fixed_interval = float(os.getenv("POLYGON_CLIENT_RETRY_INTERVAL_SEC") or 0.0)
        except (TypeError, ValueError):
            _fixed_interval = 0.0
        backoff = 0.5
        last_error = None
        ...
        for attempt in range(_max_attempts):
            try:
                ...
                if resp.status_code == 429 or resp.status_code >= 500:
                    ...
                    if _fixed_interval > 0:
                        sleep_s = _fixed_interval
                    else:
