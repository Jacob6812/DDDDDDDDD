...
def _load_prompt(prompt_name: str) -> str:
    ...
    try:
        ...
        llm_logger.warning(f"[WARN] Prompt file not found: {prompt_name}")
        return "System Role: You are a quantitative backtest assistant. Please generate JSON format responses based on input data."
...
def generate_backtest_report(
    ...
    if not llm_cfg_raw:
        llm_logger.error(
            "[ERROR] No LLM configuration found! Please specify --llm-profile parameter."
        )
        raise ValueError(
            "No LLM configuration found. Use --llm-profile parameter to specify configuration."
        )
    ...
    llm_cfg = LLMConfig(
        ...
        model=str(
            llm_cfg_raw.get(
                "backtest_report_model",
                llm_cfg_raw.get(
                    "model",
                    llm_cfg_raw.get("single_agent_model", "gpt-4o-mini"),
                ),
            )
        ),
        temperature=0.2,
        ...
        budget_completion_tokens=int(
            llm_cfg_raw.get("budget", {}).get("max_completion_tokens", 200_000)
        ),
    ...
    prompt_path = os.path.join(
        os.path.dirname(__file__), "prompts", "backtest_report_v1.txt"
    )
    try:
        with open(prompt_path, "r", encoding="utf-8") as f:
    ...
    llm_logger.info(f"[LLM] Backtest report generation - model: {llm_cfg.model}")
    ...
    if data:
        llm_logger.info(
            f"[OK] Backtest report generation successful - latency: {meta.get('latency_ms', 0)}ms, cache hit: {meta.get('cached', False)}"
        )
        llm_logger.debug(f"[RAW] LLM raw return: {raw_content[:200]}...")
    else:
        reason = meta.get("reason", "unknown")
        llm_logger.error(
            f"[ERROR] Backtest report generation failed - reason: {reason}"
        )
        if raw_content:
            llm_logger.debug(f"[RAW] LLM raw return: {raw_content[:200]}...")
    ...
    if isinstance(data, dict):
        if isinstance(data.get("report"), str):
            llm_logger.debug("[RAW] Using report field from JSON")
            return data["report"].strip()
        if isinstance(data.get("text"), str):
            llm_logger.debug("[RAW] Using text field from JSON")
        ...
        if "raw_content" in data and isinstance(data["raw_content"], str):
            llm_logger.debug("[RAW] Using raw_content field from JSON")
    ...
    try:
        ...
        if isinstance(raw, str) and raw.strip():
            ...
            if clean.strip():
                llm_logger.debug("[RAW] Fallback using raw text output")
                return clean
    except Exception:
        pass
    ...
    try:
        ...
        if reason or cached:
            print(
                f"[backtest_report_llm] LLM output not used, reason={reason}, cached={cached}"
            )
    except Exception:
        pass
    ...
    if int(m.get("trades_count", 0) or 0) < 10:
        advice.append(
            "Small sample size, recommend expanding date and symbol range to improve representativeness"
        )
    if float(m.get("volatility", 0) or 0) < 0.08 and float(m.get("sharpe", 0) or 0) > 2:
        advice.append(
            "Low volatility and high Sharpe ratio, recommend adding cost/slippage sensitivity analysis and robustness validation"
        )
    if advice:
