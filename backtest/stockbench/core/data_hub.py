...
from backtest.stockbench.adapters.finnhub_client import FinnhubClient
from backtest.stockbench.utils.io import (
    ensure_dir,
    write_parquet_idempotent,
    atomic_append_jsonl,
)
from backtest.stockbench.llm.llm_client import LLMClient, LLMConfig
...
_PARQUET_BASE = os.path.join(_STORAGE_BASE, "cache", "parquet")
...
def refresh_api_clients(
    *, polygon_api_key: str | None = None, finnhub_api_key: str | None = None
) -> None:
    global _polygon_client, _finnhub_client
    if polygon_api_key is not None:
        os.environ["POLYGON_API_KEY"] = polygon_api_key
    if finnhub_api_key is not None:
        os.environ["FINNUB_API_KEY"] = finnhub_api_key
    _polygon_client = PolygonClient(os.getenv("POLYGON_API_KEY", ""))
    _finnhub_client = FinnhubClient(
        os.getenv("FINNUB_API_KEY", ""), warn_if_missing=True
    )
...
def _show_news_cache_stats():
    ...
    try:
        ...
        logger.info(
            f"[CACHE_STATS] Found {len(cache_files)} news cache files, total cached {total_news} news items"
        )
        _cache_stats_shown = True
...
def _generate_news_cache_key(
    ticker: str, gte: str, lte: str, top_k_event_count: int, lookback_days: int
) -> str:
    """
    ...
    try:
        ...
        for d_str, items in buckets.items():
            ...
            try:
                ...
                logger.debug(
                    f"[NEWS_DAY_CACHE] {ticker}@{d_str}: +{new_count}, total {len(merged)}"
                )
            except Exception as e:
...
def _read_news_from_day_cache_range(
    ticker: str, start_str: str, end_str: str, max_items: int
) -> List[Dict]:
    ...
    try:
        ...
        for d in reversed(days):  # newest first
            ...
            try:
                ...
                try:
                    valid_items.sort(
                        key=lambda x: str(x.get("published_utc", "")), reverse=True
                    )
                except Exception:
                    pass
...
def _get_news_from_cache(
    cache_key: str, cfg: Optional[Dict] = None
) -> Optional[List[Dict]]:
    """
    ...
    # Global cache switch
    cache_mode = (
        str((cfg or {}).get("cache", {}).get("mode", "full")).lower()
        if isinstance(cfg, dict)
        else "full"
    )
    if cache_mode == "off":
...
def _save_news_to_cache(
    cache_key: str, news_items: List[Dict], cfg: Optional[Dict] = None
) -> None:
    """
...
def _nth_weekday_of_month(year: int, month: int, weekday: int, occurrence: int) -> pd.Timestamp:
    ...
    return first_day + pd.Timedelta(days=offset + (occurrence - 1) * 7)
...
def _last_weekday_of_month(year: int, month: int, weekday: int) -> pd.Timestamp:
    ...
    last_day = first_of_next_month - pd.Timedelta(days=1)
...
def _observed_fixed_holiday(year: int, month: int, day: int) -> pd.Timestamp:
    ...
    if holiday.weekday() == 5:
        return holiday - pd.Timedelta(days=1)
...
def _nyse_holidays(year: int) -> set[pd.Timestamp]:
    return {
        _observed_fixed_holiday(year, 1, 1),
        _nth_weekday_of_month(year, 1, 0, 3),
        _nth_weekday_of_month(year, 2, 0, 3),
        _easter_sunday(year) - pd.Timedelta(days=2),
...
def is_trading_day(date: pd.Timestamp) -> bool:
    """
    Determine if a given date is a NYSE trading day.
    ...
    if normalized.weekday() >= 5:
        return False
    return normalized not in _nyse_holidays(normalized.year)
...
def get_next_trading_day(date: pd.Timestamp, max_search_days: int = 10) -> pd.Timestamp:
    ...
    logger.warning(
        f"No trading day found within {max_search_days} days starting from {date}"
    )
...
def _get_trading_days_between(start_date: str, end_date: str) -> int:
    """
    Calculate the number of NYSE trading days between two dates.
    ...
    try:
        ...
        return sum(1 for day in date_range if is_trading_day(day))
    except Exception as e:
        ...
        try:
            ...
            if start_dt > end_dt:
                return 0
            date_range = pd.date_range(start=start_dt, end=end_dt, freq="D")
            return max(1, sum(1 for day in date_range if day.weekday() < 5))
        except Exception:
            return 1


def _check_data_completeness(
    df: pd.DataFrame, start_date: str, end_date: str, min_completeness: float = 0.7
) -> bool:
    """
    ...
    try:
        ...
        logger.debug(
            f"Data completeness check: expected {expected_days} days, actual {actual_days} days, completeness {completeness:.2%}"
        )
...
def _read_local_day_csv(symbol: str) -> pd.DataFrame:
    ...
    df["date"] = pd.to_datetime(df["date"], utc=True, errors="coerce").dt.tz_localize(
        None
    )
    ...
    for key, g in df.groupby(key_col):
        ...
        if dups > 0:
            _log_quality_issue(
                "duplicate_rows",
                symbol,
                {"granularity": granularity, "key": str(key), "duplicates": dups},
            )
        # Daily data only, minute sparsity check removed
        try:
            # Idempotent write by content hash (atomic replace)
            write_parquet_idempotent(
                g, fname, sort_by=[c for c in g.columns if c != key_col]
            )
        except Exception as exc:  # pragma: no cover
            logger.warning(f"write parquet failed: {fname}: {exc}")


def _filter_day_by_date(
    df: pd.DataFrame, start: str, end: str, symbol: str = "UNKNOWN"
) -> pd.DataFrame:
    if df.empty:
        return df
    ...
    if len(out) >= 2:
        ...
        if (diffs > 5).any():
            _log_quality_issue(
                "large_day_gap", symbol, {"max_gap_days": int(diffs.max())}
            )
    return out


def _read_parquet_range(
    symbol: str, granularity: str, start: str, end: str
) -> pd.DataFrame:
    try:
        base_dir = os.path.join(_PARQUET_BASE, symbol, granularity)
        if not os.path.isdir(base_dir):
        ...
        logger.warning(
            f"read parquet range failed: {symbol} {granularity} {start}-{end}: {exc}"
        )
        return pd.DataFrame([])
...
def get_bars(
    ...
    **_kwargs,
    ...
    try:
        if timespan == "day":
            ...
            if not local.empty:
                ...
                if _check_data_completeness(filtered_local, start, end):
                    logger.debug(
                        f"✅ Local Parquet data complete, return directly: {ticker}, {len(filtered_local)} records"
                    )
                    return filtered_local
                else:
                    logger.info(
                        f"⚠️ Local Parquet data incomplete, will try other sources: {ticker}, current {len(filtered_local)} records"
                    )
            ...
            if not df.empty:
                ...
                if _check_data_completeness(filtered_csv, start, end):
                    logger.debug(
                        f"✅ Local CSV data complete, return directly: {ticker}, {len(filtered_csv)} records"
                    )
                    return filtered_csv
                else:
                    logger.info(
                        f"⚠️ Local CSV data incomplete, will try other sources: {ticker}, current {len(filtered_csv)} records"
                    )
            ...
            if not api_available:
                ...
                logger.info(
                    f"🔄 [API-UNAVAILABLE] API not available, returning best cached data"
                )
                if not local.empty:
                    fallback_local = _filter_day_by_date(local, start, end, ticker)
                    logger.info(
                        f"📊 [CACHE-FALLBACK] Returning incomplete Parquet data: {ticker}, {len(fallback_local)} records (API unavailable)"
                    )
                    return fallback_local
                elif not df.empty:
                    fallback_csv = _filter_day_by_date(df, start, end, ticker)
                    logger.info(
                        f"📊 [CACHE-FALLBACK] Returning incomplete CSV data: {ticker}, {len(fallback_csv)} records (API unavailable)"
                    )
                    return fallback_csv
                else:
                    logger.info(
                        f"[OFFLINE_ONLY] No cached data available for {ticker} {start}-{end}"
                    )
                    return _empty_bars_df()

            # 3) API is available: try to fetch complete data
            logger.info(
                f"🌐 [API-PRIORITY] API available, attempting to fetch complete data: {ticker}, {start} to {end}"
            )
            try:
                aggs = _polygon_client.list_aggs(
                    ticker, start, end, multiplier=1, timespan="day", adjusted=adjusted
                )
                if not aggs:
                    logger.warning(
                        f"API returned no data: {ticker}, falling back to cached data"
                    )
                    # API returned no data: try cached data as fallback
                    if not local.empty:
                        fallback_local = _filter_day_by_date(local, start, end, ticker)
                        logger.info(
                            f"📊 [API-FALLBACK] Returning cached Parquet data: {ticker}, {len(fallback_local)} records (API empty)"
                        )
                        return fallback_local
                    elif not df.empty:
                        fallback_csv = _filter_day_by_date(df, start, end, ticker)
                        logger.info(
                            f"📊 [API-FALLBACK] Returning cached CSV data: {ticker}, {len(fallback_csv)} records (API empty)"
                        )
                        return fallback_csv
                    else:
                        return _empty_bars_df()
                ...
                df = pd.DataFrame(
                    [
                        {
                            "date": pd.to_datetime(x.get("t"), unit="ms", utc=True)
                            .tz_localize(None)
                            .date(),
                            "open": x.get("o"),
                            "high": x.get("h"),
                ...
                logger.info(
                    f"🔄 [API-FAILED] Falling back to cached data after API failure"
                )
                # API call failed: try cached data as fallback
                if not local.empty:
                    fallback_local = _filter_day_by_date(local, start, end, ticker)
                    logger.info(
                        f"📊 [API-FALLBACK] Returning cached Parquet data: {ticker}, {len(fallback_local)} records (API failed)"
                    )
                    return fallback_local
                elif not df.empty:
                    fallback_csv = _filter_day_by_date(df, start, end, ticker)
                    logger.info(
                        f"📊 [API-FALLBACK] Returning cached CSV data: {ticker}, {len(fallback_csv)} records (API failed)"
                    )
                    return fallback_csv
                else:
        ...
        try:
            logger.info(
                f"🔄 [ERROR-RECOVERY] Attempting to return cached data after error"
            )
            local_recovery = _read_parquet_range(ticker, "day", start, end)
            if not local_recovery.empty:
                filtered_recovery = _filter_day_by_date(
                    local_recovery, start, end, ticker
                )
                logger.info(
                    f"📊 [ERROR-RECOVERY] Returning {len(filtered_recovery)} cached records despite error"
                )
...
def get_grouped_daily(
    date: str, adjusted: bool, cfg: Optional[Dict] = None, **_kwargs
) -> pd.DataFrame:
    try:
...
def get_universal_snapshots(
    symbols: List[str], cfg: Optional[Dict] = None, **_kwargs
) -> Dict[str, Dict]:
    try:
        if _is_offline_only(cfg):
...
def get_gainers_losers(
    top_n: int, cfg: Optional[Dict] = None, **_kwargs
) -> Dict[str, List[str]]:
    try:
        if _is_offline_only(cfg):
...
def get_news(
    ...
    **_kwargs,
    ...
    try:
        ...
        logger.info(
            f"📋 Start getting news - Stock: {ticker}, GTE: {gte}, LTE: {lte}, limit: {limit}, page_token: {page_token}"
        )
        ...
        target_news_count = (
            cfg.get("news", {}).get("top_k_event_count", 5) if cfg else 5
        )
        # Get lookback days from configuration, default to 3 days
        lookback_days = cfg.get("news", {}).get("lookback_days", 3) if cfg else 3
        logger.debug(
            f"📋 Configuration parameters - Target news count: {target_news_count}, Lookback days: {lookback_days}"
        )
        ...
        if lte and gte:
            ...
            if raw_start_date == raw_end_date:
                ...
                logger.debug(
                    f"📅 Single day decision query: decision_date={decision_date.strftime('%Y-%m-%d')}, news_range={start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
                )
            else:
                ...
                if start_date > end_date:
                    ...
                    logger.warning(
                        f"📅 Date range adjusted to prevent lookahead bias: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
                    )
        elif lte:
        ...
        day_cached = _read_news_from_day_cache_range(
            ticker, start_str, end_str, max_items=target_news_count
        )
        if day_cached and len(day_cached) >= target_news_count:
            logger.info(
                f"✅ [NEWS_DAY_CACHE] Hit day cache with sufficient data: {ticker}, {len(day_cached)}/{target_news_count}"
            )
            return day_cached, None
        elif day_cached:
            logger.info(
                f"⚠️ [NEWS_DAY_CACHE] Found partial cache data: {ticker}, {len(day_cached)}/{target_news_count}, will try API"
            )

        # 0.5) Deterministic hash cache (legacy) then write-through to day cache before return
        cache_key = _generate_news_cache_key(
            ticker, gte, lte, target_news_count, lookback_days
        )
        ...
        if cached_result is not None and len(cached_result) >= target_news_count:
            logger.info(
                f"✅ Get news from hash cache with sufficient data: {ticker}, key: {cache_key[:16]}..., count: {len(cached_result)}"
            )
            try:
            ...
            logger.info(
                f"⚠️ Hash cache has partial data: {ticker}, {len(cached_result)}/{target_news_count}, will try API"
            )
        else:
            logger.debug(f"⚠️ Cache miss, need to fetch via API")

        # Check API availability before deciding strategy
        api_available = not _is_offline_only(cfg) and (
            _finnhub_client.is_available() or _polygon_client.api_key
        )

        if not api_available:
            # API not available: return best available cached data
            logger.info(
                f"🔄 [API-UNAVAILABLE] API not available for news, returning best cached data"
            )

            # First try previously found partial cache data
            if day_cached:
                logger.info(
                    f"📊 [CACHE-FALLBACK] Returning day cache data: {ticker}, {len(day_cached)} items (API unavailable)"
                )
                return day_cached, None
            elif cached_result is not None:
                logger.info(
                    f"📊 [CACHE-FALLBACK] Returning hash cache data: {ticker}, {len(cached_result)} items (API unavailable)"
                )
                return cached_result, None

            # Try extended range search as last resort
            extended_start = (pd.to_datetime(start_str) - timedelta(days=2)).strftime(
                "%Y-%m-%d"
            )
            extended_end = (pd.to_datetime(end_str) + timedelta(days=2)).strftime(
                "%Y-%m-%d"
            )
            fallback_cached = _read_news_from_day_cache_range(
                ticker, extended_start, extended_end, max_items=target_news_count * 2
            )

            if fallback_cached:
                logger.info(
                    f"📊 [CACHE-FALLBACK] Returning extended cache data: {ticker}, {len(fallback_cached)} items (API unavailable)"
                )
                return fallback_cached[:target_news_count], None
            else:
                logger.info(
                    f"[OFFLINE_ONLY] No cached news available for {ticker} {start_str}-{end_str}"
                )
                return [], None

        # Validate time range (for debugging)
        logger.info(
            f"📅 News time range: {ticker}, Original end date: {raw_end_date.strftime('%Y-%m-%d')}, "
        ...
        logger.info(
            f"🔧 [Finnhub] Client status: {'available' if finnhub_available else 'unavailable'}"
        )

        if finnhub_available:
            logger.info(
                f"🔍 [Finnhub] Start getting news: {ticker}, time range: {start_str} to {end_str}, request count: {target_news_count * 2}"
            )
            try:
                finnhub_news = _finnhub_client.get_company_news(
                    ticker, start_str, end_str, limit=target_news_count * 2
                )

                if finnhub_news:
                    # Record raw data statistics
                    logger.debug(
                        f"📊 [Finnhub] Raw response: {len(finnhub_news)} news items"
                    )
                    ...
                    for item in finnhub_news:
                        ...
                        if title or description:  # At least one is not empty
                            ...
                            invalid_count += 1
                            logger.debug(
                                f"🚫 [Finnhub] Invalid news item: title='{title}', desc='{description}'"
                            )

                    all_news_items.extend(valid_finnhub_news)
                    logger.info(
                        f"✅ [Finnhub] Successfully got {len(valid_finnhub_news)} valid news items (raw: {len(finnhub_news)}, invalid: {invalid_count})"
                    )

                    # Record time distribution
                    if valid_finnhub_news:
                        times = [
                            item.get("published_utc", "") for item in valid_finnhub_news
                        ]
                        valid_times = [t for t in times if t]
                        if valid_times:
                            logger.debug(
                                f"📅 [Finnhub] News time distribution: {min(valid_times)} to {max(valid_times)}"
                            )
                else:
                    logger.warning(f"⚠️ [Finnhub] No news obtained: {ticker}")
            except Exception as finnhub_exc:
                logger.error(
                    f"❌ [Finnhub] Exception occurred while getting news: {finnhub_exc}",
                    exc_info=True,
                )
        else:
        ...
        if len(all_news_items) < target_news_count and _polygon_client.api_key:
            ...
            logger.info(
                f"📊 News count insufficient ({len(all_news_items)}/{target_news_count}), start Polygon supplement"
            )
            logger.info(
                f"🔍 [Polygon] Start getting news: {ticker}, need {needed_count} items"
            )
            ...
            try:
                ...
                while (
                    len(polygon_news_items) < needed_count * 2
                ):  # Get more, then filter
                    api_call_count += 1
                    logger.debug(
                        f"🌐 [Polygon] API call #{api_call_count}, cursor: {cursor}"
                    )
                    ...
                    try:
                        ...
                        if not items:
                            logger.debug(
                                f"🔚 [Polygon] Call #{api_call_count} returned empty result, end fetch"
                            )
                            break

                        total_fetched += len(items)
                        logger.debug(
                            f"📊 [Polygon] Call #{api_call_count} got {len(items)} raw news items"
                        )
                        ...
                        logger.debug(
                            f"✅ [Polygon] Call #{api_call_count}: valid {len(valid_items)}, invalid {invalid_items}"
                        )
                        ...
                        logger.error(
                            f"❌ [Polygon] API call #{api_call_count} failed: {api_exc}"
                        )
                        break
                ...
                if polygon_news_items:
                    logger.debug(
                        f"📊 [Polygon] News count before sorting: {len(polygon_news_items)}"
                    )
                    polygon_news_items.sort(
                        key=lambda x: x.get("published_utc", ""), reverse=True
                    )

                    # Record time distribution
                    times = [
                        item.get("published_utc", "") for item in polygon_news_items
                    ]
                    valid_times = [t for t in times if t]
                    if valid_times:
                        logger.debug(
                            f"📅 [Polygon] News time distribution: {min(valid_times)} to {max(valid_times)}"
                        )
                    ...
                    logger.debug(
                        f"🎯 [Polygon] Cut to needed count: {len(polygon_news_items)}/{original_count}"
                    )
                    ...
                    for item in polygon_news_items:
                        ...
                        if title not in existing_titles:
                            ...
                            duplicates_count += 1
                            logger.debug(
                                f"🔄 [Polygon] Found duplicate news: {title[:50]}..."
                            )

                    all_news_items.extend(unique_polygon_items)
                    logger.info(
                        f"✅ [Polygon] Supplement complete - API calls: {api_call_count}, raw: {total_fetched}, valid: {len(polygon_news_items)}, dedup: {duplicates_count}, final added: {len(unique_polygon_items)}"
                    )
                else:
                    logger.warning(
                        f"⚠️ [Polygon] Still no valid news after {api_call_count} calls: {ticker}"
                    )

            except Exception as exc:
                logger.error(
                    f"❌ [Polygon] Exception occurred while getting news: {exc}",
                    exc_info=True,
                )
        elif len(all_news_items) >= target_news_count:
            logger.info(
                f"✅ [Finnhub] Requirements satisfied, skip Polygon: {len(all_news_items)}/{target_news_count} items"
            )
        else:
            logger.warning(
                f"⚠️ [API-UNAVAILABLE] Both Finnhub and Polygon APIs unavailable, using existing news: {len(all_news_items)} items"
            )
        ...
        logger.debug(
            f"🎯 Final count limit: {len(final_news_items)}/{original_total} (target: {target_news_count})"
        )
        ...
        finnhub_count = sum(
            ...
            if item.get("api_source", "").startswith("finnhub")
        )
        polygon_count = sum(
            1 for item in final_news_items if item.get("api_source", "") == "polygon"
        )
        ...
        if final_news_items:
            titles_count = sum(
                1 for item in final_news_items if item.get("title", "").strip()
            )
            descriptions_count = sum(
                1 for item in final_news_items if item.get("description", "").strip()
            )
            logger.debug(
                f"📊 Data quality statistics: has title {titles_count}/{len(final_news_items)}, has description {descriptions_count}/{len(final_news_items)}"
            )
        ...
        if len(final_news_items) < target_news_count:
            logger.warning(
                f"📰 News fetch incomplete: {ticker}, got {len(final_news_items)}/{target_news_count} | Data source: [{source_summary}]"
            )
            ...
            if reasons:
                logger.debug(
                    f"🔍 Possible reasons for insufficient fetch: {', '.join(reasons)}"
                )
        else:
            logger.info(
                f"📰 News fetch complete: {ticker}, successfully got target count: {len(final_news_items)}/{target_news_count} | Data source distribution: [{source_summary}]"
            )
        ...
        logger.error(
            f"❌ get_news function execution failed - Stock: {ticker}, Error: {exc}",
            exc_info=True,
        )
        # Record detailed context on failure
        logger.debug(
            f"🔍 Failure context: gte={gte}, lte={lte}, limit={limit}, cfg={cfg}"
        )

        # Try to return cached data as last resort
        logger.info(
            f"🔄 [ERROR-RECOVERY] Attempting to return cached news data after error"
        )
        try:
            # First try previously found partial cache data
            if day_cached:
                logger.info(
                    f"📊 [ERROR-RECOVERY] Returning day cache data: {ticker}, {len(day_cached)} items despite error"
                )
                return day_cached, None
            elif cached_result is not None:
                logger.info(
                    f"📊 [ERROR-RECOVERY] Returning hash cache data: {ticker}, {len(cached_result)} items despite error"
                )
                return cached_result, None
        except:
            pass
...
def get_ticker_details(
    ticker: str, date: Optional[str] = None, cfg: Optional[Dict] = None, **_kwargs
) -> Dict:
    try:
        if _is_offline_only(cfg):
...
def compare_with_legacy_day(
    symbol: str, start: str, end: str, tolerance: float = 1e-6
) -> Dict[str, object]:
    """Align local CSV (legacy) with current Parquet (daily) format, output difference report and save to reports/alignments.
    ...
    try:
        ...
        current = (
            ...
            if rows
            else pd.DataFrame(columns=legacy.columns)
        )
        ...
        merged = legacy.merge(
            current, on="date", how="inner", suffixes=("_legacy", "_cur")
        )
    ...
    **_kwargs,
    ...
    try:
        ...
        if use_cache and os.path.exists(cache_path):
            # Cache is permanently valid (no TTL).
            try:
                with open(cache_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
        ...
        items = _polygon_client.list_financials(
            ticker, timeframe=timeframe, limit=limit
        )
    ...
    try:
                ...
                if not is_today:
                    with open(cache_path, "r", encoding="utf-8") as f:
                        ...
                        if isinstance(cached_data, dict):
                            logger.debug(
                                f"Using cached stock metrics data (historical): {ticker}@{cache_date}"
                            )
                            return cached_data
                else:
                    ...
                    if (datetime.now().timestamp() - st.st_mtime) <= 12 * 3600:
                        with open(cache_path, "r", encoding="utf-8") as f:
                            ...
                            if isinstance(cached_data, dict):
                                logger.debug(
                                    f"Using cached stock metrics data (today, within TTL): {ticker}"
                                )
                                return cached_data
                    else:
        ...
        if _is_offline_only(cfg):
            try:
                ...
                logger.warning(
                    f"[OFFLINE_ONLY] Failed to read precomputed indicators: {ticker}@{cache_date}: {_off_exc}"
                )
            return {
                "market_cap": 0.0,
                "pe_ratio": 0.0,
        ...
        logger.info(
            f"🚀 [FUNDAMENTAL_DATA] Attempting to get stock indicators for {ticker}"
        )
        indicators = None
        data_source = "unknown"
        ...
        if _finnhub_client.is_available():
            try:
                logger.info(
                    f"📡 [FUNDAMENTAL_DATA] Calling Finnhub client with ticker={ticker}, date={date}"
                )
                indicators = _finnhub_client.get_stock_indicators(ticker, date)
                if indicators and any(
                    indicators.get(key, 0) > 0
                    for key in ["market_cap", "pe_ratio", "week_52_high", "week_52_low"]
                ):
                    data_source = "finnhub"
                    logger.info(
                        f"✅ [FUNDAMENTAL_DATA] Finnhub successfully returned data for {ticker}"
                    )
                else:
                    logger.warning(
                        f"⚠️ [FUNDAMENTAL_DATA] Finnhub returned empty/invalid data for {ticker}"
                    )
                    indicators = None
            except Exception as e:
                logger.warning(
                    f"❌ [FUNDAMENTAL_DATA] Finnhub failed for {ticker}: {e}"
                )
                indicators = None
        else:
        ...
        if indicators is None and _polygon_client.api_key:
            ...
            try:
                logger.info(
                    f"📡 [FUNDAMENTAL_DATA] Calling Polygon client with ticker={ticker}, date={date}"
                )
                ...
                if indicators and any(
                    ...
                    data_source = "polygon"
                    logger.info(
                        f"✅ [FUNDAMENTAL_DATA] Polygon successfully returned data for {ticker}"
                    )
                else:
                    logger.warning(
                        f"⚠️ [FUNDAMENTAL_DATA] Polygon returned empty/invalid data for {ticker}"
                    )
                    indicators = None
            except Exception as e:
                logger.warning(
                    f"❌ [FUNDAMENTAL_DATA] Polygon failed for {ticker}: {e}"
                )
                indicators = None
        ...
        if indicators is None:
            logger.info(
                f"🔄 [API-FAILED] Both APIs failed/unavailable for {ticker}, trying cached data"
            )
            ...
            try:
                if os.path.exists(cache_path):
                    with open(cache_path, "r", encoding="utf-8") as f:
                        cached_data = json.load(f)
                    if isinstance(cached_data, dict) and any(
                        cached_data.get(key, 0) > 0
                        for key in [
                            "market_cap",
                            "pe_ratio",
                            "week_52_high",
                        ...
                        logger.info(
                            f"📊 [CACHE-FALLBACK] Using cached indicators for {ticker}"
                        )
            ...
            if indicators is None:
                logger.warning(
                    f"⚠️ [FUNDAMENTAL_DATA] No cached data available for {ticker}, using default values"
                )
                indicators = {
                    "market_cap": 0.0,
                    "pe_ratio": 0.0,
        ...
        if indicators:
            ...
            logger.info(
                f"📊 [FUNDAMENTAL_DATA] Final indicators for {ticker} from {data_source}: market_cap={indicators.get('market_cap', 0):.0f}"
            )
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Failed to cache stock metrics data: {ticker}, {cache_exc}"
            )

        logger.info(
            f"✅ [FUNDAMENTAL_DATA] Final result for {ticker} (source: {data_source}):"
        )
        ...
        logger.info(
            f"  - Quarterly Dividend: ${indicators.get('quarterly_dividend', 0):.4f}"
        )
        ...
        logger.error(
            f"❌ [FUNDAMENTAL_DATA] Failed to get stock metrics: {ticker}, {exc}"
        )
...
def clear_old_news_cache() -> None:
    ...
    try:
        ...
        for filename in os.listdir(cache_dir):
            ...
            filepath = os.path.join(cache_dir, filename)
            try:
                # Only check if file is corrupted; do not clean based on time
                with open(filepath, "r", encoding="utf-8") as f:
    ...
    try:
        ...
        for filename in os.listdir(cache_dir):
            ...
            try:
                ...
                cache_files.append(
                    {"filename": filename, "error": str(e), "format": "corrupted"}
                )
