...
_NEGATIVE_SHARE_RESIDUAL_EPSILON = 0.010001


def _normalize_position_residual(position: "Position") -> None:
    shares = float(getattr(position, "shares", 0.0) or 0.0)
    if abs(shares) <= _POSITION_EPSILON or (shares < 0.0 and abs(shares) <= _NEGATIVE_SHARE_RESIDUAL_EPSILON):
        position.shares = 0.0
        position.avg_price = 0.0
        position.holding_days = 0
        position.total_cost = 0.0

from backtest.stockbench.metrics import evaluate
from backtest.stockbench.slippage import Slippage
from backtest.stockbench.metrics import compute_nav_to_metrics_series
...
class Portfolio:
    ...
    def equity(
        ...
        previous_open_prices: Dict[str, float] = None,
    ) -> float:
        ...
        if open_prices is None:
            ...
            if previous_open_prices is None:
                raise ValueError(
                    "Must provide either open_prices or previous_open_prices"
                )
            ...
            return self.cash + self.get_total_position_value(
                open_prices, previous_open_prices
            )
    ...
    def get_position_pct(
        self, symbol: str, open_price: float, open_prices: Dict[str, float] = None
    ) -> float:
        ...
        if open_prices is None:
            ...
            raise ValueError(
                "Must provide open_prices dictionary for position percentage calculation"
            )
        else:
            open_prices_dict = open_prices.copy()
        open_prices_dict[symbol] = (
            open_price  # Ensure current symbol's price is correct
        )
    ...
    def update_cash(self, amount: float) -> bool:
        ...
        logger.info(
            f"[CASH_UPDATE] Change amount: {amount:.2f} ({'Increase' if amount >= 0 else 'Decrease'})"
        )

        new_cash = self.cash + amount
        logger.debug(
            f"[CASH_UPDATE] Calculate new cash: {self.cash:.2f} + ({amount:.2f}) = {new_cash:.2f}"
        )

        if new_cash < 0:
            logger.warning(
                f"[CASH_PROTECTION] Cash update rejected: new cash {new_cash:.2f} < 0"
            )
...
def load_benchmark_components(
    bench_cfg: Dict, datasets, dates: pd.DatetimeIndex, field: str = "adjusted_close"
) -> Dict[str, pd.Series]:
    ...
    for sym in symbols:
        ...
        ser = ser[
            (ser.index >= pd.to_datetime(start)) & (ser.index <= pd.to_datetime(end))
        ]
        ser = ser.dropna()
        if len(ser) == 0:
            continue
...
def _load_symbol_price_series(
    datasets, symbols: List[str], start: str, end: str, field: str = "open"
) -> Dict[str, pd.Series]:
    ...
    for sym in symbols:
        try:
            ...
            ser = ser[~ser.index.duplicated(keep="last")]
            ser = ser[
                (ser.index >= pd.to_datetime(start))
                & (ser.index <= pd.to_datetime(end))
            ]
            ser = ser.dropna()
            if len(ser) == 0:
                continue
...
def build_per_symbol_bh_benchmark(
    ...
    # Read prices
    series_map = _load_symbol_price_series(
        datasets=cfg.get("_datasets"),
        symbols=symbols,
        start=start,
        end=end,
        field=price_field,
    ...
    C_total = (
        float(total_cash_override)
        if (total_cash_override is not None)
        else portfolio_cash
    )
...
def aggregate_with_rebalance(
    returns_df: pd.DataFrame, basket: List[Dict], freq: str = "daily"
) -> pd.Series:
    ...
    w = pd.Series(
        {c: weights_map.get(c, 0.0) for c in returns_df.columns},
...
def align_with_strategy_nav(
    bench_nav: pd.Series,
    dates: pd.DatetimeIndex,
    reindex: str = "inner_join",
    fill: str = "ffill",
    fill_limit: int = 0,
) -> pd.Series:
    ...
    try:
        ...
        if fill_mode == "ffill":
            bench = bench.ffill(
                limit=None if int(fill_limit or 0) <= 0 else int(fill_limit)
            )
        bench = bench.dropna()
...
class BacktestEngine:
    def __init__(self, cfg: Dict, datasets, slippage_model: Slippage) -> None:
        ...
        self.rebalance_at_open = bool(
            (cfg.get("backtest", {}) or {}).get("rebalance_at_open", True)
        )
        ...
        self.enable_detailed_logging = bool(
            cfg.get("backtest", {}).get("enable_detailed_logging", False)
        )

        # Get initial capital, prioritize using portfolio.total_cash
        portfolio_cash = float(
            self.cfg.get("portfolio", {}).get("total_cash", 1_000_000)
        )
        self.initial_cash = float(
            self.cfg.get("backtest", {}).get("cash", portfolio_cash)
        )

        # Re-decision deduplication flag (avoid infinite triggering of same symbol on same day/minute)
        self._redecide_flags = set()

    def _resume_backtest_cfg(self) -> Dict[str, Any]:
        resume_cfg = (self.cfg.get("backtest", {}) or {}).get("resume", {})
        return resume_cfg if isinstance(resume_cfg, dict) else {}

    def _coerce_resume_snapshot(
        self,
        ...
        return PortfolioSnapshot(
            ...
            date=str(snapshot.get("date") or fallback_date),
            cash=float(snapshot.get("cash", row.get("cash", 0.0)) or 0.0),
            total_equity=float(
                snapshot.get("total_equity", row.get("total_equity", 0.0)) or 0.0
            ),
            total_position_value=float(
                snapshot.get(
                    "total_position_value",
                    row.get("mark_to_market", 0.0),
                )
                or 0.0
            ),
            unrealized_pnl=float(snapshot.get("unrealized_pnl", 0.0) or 0.0),
            nav=float(snapshot.get("nav", row.get("daily_nav", 1.0)) or 1.0),
            positions=dict(snapshot.get("positions") or {}),
            benchmark_nav=float(snapshot.get("benchmark_nav", 0.0) or 0.0),
        )

    def _load_resume_state(self) -> Dict[str, Any] | None:
        resume_cfg = self._resume_backtest_cfg()
        if not bool(resume_cfg.get("enabled")):
            return None
        ...
        if not summary_path_raw:
            raise ValueError(
                "backtest.stockbench.resume.enabled requires daily_summary_path or snapshot_path"
            )

        summary_path = Path(str(summary_path_raw))
        if not summary_path.exists():
            raise FileNotFoundError(
                f"Resume daily summary not found: {summary_path}"
            )
        ...
        if not rows:
            raise ValueError(f"Resume daily summary is empty: {summary_path}")

        resume_trade_date = str(resume_cfg.get("trade_date") or "").strip()
        ...
        if resume_trade_date:
            matching_index = next(
                (
                    ...
                    if str(row.get("trade_date") or row.get("date") or "")
                    == resume_trade_date
                ),
                None,
            )
            if matching_index is None:
                raise ValueError(
                    f"Resume trade_date {resume_trade_date} not found in {summary_path}"
                )
            selected_index = matching_index

        selected_row = rows[selected_index]
        snapshot = dict(selected_row.get("portfolio_snapshot") or {})
        positions_raw = snapshot.get("positions") or {}
        positions: Dict[str, Position] = {}
        for symbol, raw_info in positions_raw.items():
            ...
            positions[str(symbol)] = Position(
                ...
                holding_days=int(info.get("holding_days", 0) or 0),
                total_cost=float(info.get("total_cost", 0.0) or 0.0),
            )

        portfolio = Portfolio(
            cash=float(snapshot.get("cash", selected_row.get("cash", self.initial_cash)) or 0.0),
            positions=positions,
        )
        historical_nav = [
            {
                "date": pd.Timestamp(str(row.get("trade_date") or row.get("date"))),
                "nav": float(row.get("daily_nav", 0.0) or 0.0),
            }
            for row in rows[: selected_index + 1]
            if str(row.get("trade_date") or row.get("date") or "")
        ]
        historical_snapshots = [
            self._coerce_resume_snapshot(
                dict(row.get("portfolio_snapshot") or {}),
                str(row.get("trade_date") or row.get("date") or ""),
                row,
            )
            for row in rows[: selected_index + 1]
            if isinstance(row.get("portfolio_snapshot"), dict)
        ]
        executed_symbols_by_date = {
            str(row.get("trade_date") or row.get("date") or ""): set(
                str(symbol) for symbol in (row.get("executed_symbols") or [])
            )
            for row in rows[: selected_index + 1]
        }
        historical_trade_rows: List[Dict[str, Any]] = []
        decisions_path = summary_path.parent / "darwintrade_portfolio_decisions.daily.jsonl"
        if decisions_path.exists():
            with decisions_path.open("r", encoding="utf-8") as fh:
                for line in fh:
                    ...
                    if not trade_date or trade_date not in executed_symbols_by_date:
                        continue
                    if trade_date > str(selected_row.get("trade_date") or selected_row.get("date") or ""):
                        continue
                    symbol = str(decision.get("symbol") or "")
                    if symbol not in executed_symbols_by_date.get(trade_date, set()):
                        continue
                    semantic_action = str(decision.get("semantic_action") or "")
                    action = str(decision.get("action") or "")
                    if semantic_action in {"portfolio_no_trade", "portfolio_no_trade_asset"}:
                        continue
                    if action == "hold":
                        continue
                    sim_order = decision.get("sim_order") if isinstance(decision.get("sim_order"), dict) else {}
                    quantity = float(
                        sim_order.get("quantity", decision.get("shares", 0.0)) or 0.0
                    )
                    reference_price = float(
                        decision.get("reference_price", sim_order.get("reference_price", 0.0)) or 0.0
                    )
                    if quantity <= 0.0 or reference_price <= 0.0:
                        continue
                    side = str(sim_order.get("side") or "buy")
                    filled_qty = round(
                        quantity * max(0.0, min(1.0, self.fill_ratio)),
                        2,
                    )
                    if filled_qty <= 0.0:
                        continue
                    exec_price = (
                        self.slippage.apply_sell(reference_price)
                        if side in {"sell", "sell_short"}
                        else self.slippage.apply_buy(reference_price)
                    )
                    historical_trade_rows.append(
                        {
        ...
        return {
            "portfolio": portfolio,
            "nav": historical_nav,
            "portfolio_snapshots": historical_snapshots,
            "trade_rows": historical_trade_rows,
            "resume_row": selected_row,
            "summary_path": str(summary_path),
        }

    def _validate_portfolio_position_consistency(
        self, pf: Portfolio, ctx: Dict, date: pd.Timestamp
    ) -> None:
        """
        ...
        try:
            logger.info(
                f"[POSITION_VALIDATION] {date.date()}: Validating portfolio position consistency"
            )
            ...
            for symbol, position in pf.positions.items():
                ...
                _normalize_position_residual(position)
                ...
                if shares != 0 and avg_price <= 0:
                    logger.error(
                        f"[POSITION_VALIDATION] {symbol}: Has shares ({shares}) but invalid avg_price ({avg_price})"
                    )
                    inconsistencies_found += 1

                if shares == 0 and (avg_price > 0 or holding_days > 0):
                    logger.warning(
                        f"[POSITION_VALIDATION] {symbol}: Zero shares but avg_price={avg_price}, holding_days={holding_days}"
                    )

                # Check for the specific 57.01 bug
                if abs(shares - 57.01) < 0.001:
                    logger.error(
                        f"[POSITION_VALIDATION] {symbol}: Suspicious shares value 57.01 detected! This may be the target_cash_amount/price bug."
                    )
                    inconsistencies_found += 1

                logger.debug(
                    f"[POSITION_VALIDATION] {symbol}: shares={shares:.2f}, avg_price={avg_price:.4f}, holding_days={holding_days}"
                )

            if inconsistencies_found > 0:
                logger.error(
                    f"[POSITION_VALIDATION] Found {inconsistencies_found} position inconsistencies on {date.date()}"
                )
            else:
                logger.info(
                    f"[POSITION_VALIDATION] {date.date()}: All positions validated successfully"
                )

        except Exception as e:
            logger.warning(f"[POSITION_VALIDATION] Validation failed: {e}")

    def _maybe_redecide_qty(
        self,
        strategy,
        ctx: Dict,
        symbol: str,
        current_shares: float,
        proposed_filled_qty: float,
        key: Tuple,
    ) -> float | None:
    ...
    def _create_portfolio_snapshot(
        ...
        position_value = pf.get_total_position_value(
            open_prices, previous_open_prices
        )  # Position value = opening price market value
        total_assets = (
        ...
        for symbol, pos in pf.positions.items():
            if pos.shares != 0:
                ...
                position_pct = (
                    pf.get_position_pct(symbol, open_price, price_for_pct)
                    if price_for_pct
                    else 0.0
                )
    ...
    def _get_next_day_open_prices(
        self, next_date: pd.Timestamp, symbols: List[str]
    ) -> Dict[str, float]:
        ...
        for day_offset in range(max_days_ahead):
            logger.debug(
                f"[NEXT_DAY_PRICE] Trying to fetch open prices for {current_date.strftime('%Y-%m-%d')} (attempt {day_offset + 1})"
            )
            ...
            for s in symbols:
                if s not in next_open_map:  # Only fetch for symbols without prices yet
                    try:
                        bars = self.datasets.get_day_bars(
                            s,
                        ...
                        if not bars.empty:
                            ...
                            day_success_count += 1
                            logger.debug(
                                f"[NEXT_DAY_PRICE] {current_date.strftime('%Y-%m-%d')} {s}: got opening price {open_px}"
                            )
                        else:
                            logger.debug(
                                f"[NEXT_DAY_PRICE] {current_date.strftime('%Y-%m-%d')} {s}: empty daily bars (possibly a holiday)"
                            )
                    except Exception as e:
                        logger.warning(
                            f"[NEXT_DAY_PRICE] {current_date.strftime('%Y-%m-%d')} {s}: failed to get opening price - {e}"
                        )
        ...
        logger.info(
            f"[NEXT_DAY_PRICE] Final result: successfully fetched opening prices for {len(next_open_map)}/{len(symbols)} symbols"
        )
        if len(next_open_map) < len(symbols):
            missing_symbols = [s for s in symbols if s not in next_open_map]
            logger.warning(
                f"[NEXT_DAY_PRICE] Warning: failed to get opening prices for the following symbols: {missing_symbols}"
            )
    ...
    def _create_trade_record(
        ...
        open_prices: Dict[str, float],
        ts: pd.Timestamp,
        pre_trade_shares: float | None = None,
        pre_trade_avg_price: float | None = None,
        pre_trade_holding_days: int | None = None,
        pre_trade_cash: float | None = None,
    ) -> TradeRecord:
        """Create a detailed trade record"""
        pos = pf.positions.get(symbol)

        # Pre-trade state
        cash_before = float(pre_trade_cash) if pre_trade_cash is not None else pf.cash + net_cost
        position_before = float(pre_trade_shares) if pre_trade_shares is not None else pos.shares - filled_qty if pos else -filled_qty
        avg_price_before = float(pre_trade_avg_price) if pre_trade_avg_price is not None else pos.avg_price if pos and pos.shares != filled_qty else 0.0
        ...
        if symbol in temp_pf.positions:
            temp_pf.positions[symbol] = Position(
                ...
                holding_days=int(pre_trade_holding_days) if pre_trade_holding_days is not None else pos.holding_days if pos else 0,
            )

        # Temporarily use None for previous_open_prices because this is mainly a trade record
        total_equity_before = (
            temp_pf.get_total_position_value(open_prices, None) + cash_before
        )
        total_position_value_before = temp_pf.get_total_position_value(
            open_prices, None
        )
        ...
        for sym, pos_temp in temp_pf.positions.items():
            if pos_temp.shares != 0:
                ...
                unrealized_pnl_before += pos_temp.shares * (
                    mark_price - pos_temp.avg_price
                )
        ...
        for sym, pos_temp in pf.positions.items():
            if pos_temp.shares != 0:
                # Price fallback priority: opening price -> cost price (use opening price for valuation)
                mark_price = open_prices.get(sym, pos_temp.avg_price)
                unrealized_pnl_after += pos_temp.shares * (
                    mark_price - pos_temp.avg_price
                )

        # Compute realized P&L when reducing an existing long or short position.
        realized_pnl = 0.0
        closing_qty = min(abs(filled_qty), abs(position_before)) if position_before * filled_qty < 0 else 0.0
        if closing_qty > 0.0 and position_before > 0:
            realized_pnl = closing_qty * (exec_price - avg_price_before)
        elif closing_qty > 0.0 and position_before < 0:
            realized_pnl = closing_qty * (avg_price_before - exec_price)
        ...
        try:
            ...
            with open(trades_file, "w", encoding="utf-8") as f:
                for record in trade_records:
                    f.write(
                        json.dumps(record.__dict__, ensure_ascii=False, default=str)
                        + "\n"
                    )

            # Save portfolio snapshots
            snapshots_file = os.path.join(
                output_dir, "detailed_portfolio_snapshots.jsonl"
            )
            with open(snapshots_file, "w", encoding="utf-8") as f:
                for snapshot in portfolio_snapshots:
                    f.write(
                        json.dumps(snapshot.__dict__, ensure_ascii=False, default=str)
                        + "\n"
                    )
            ...
            summary = {
                ...
                "final_cash": portfolio_snapshots[-1].cash
                if portfolio_snapshots
                else 0.0,
                "final_equity": portfolio_snapshots[-1].total_equity
                if portfolio_snapshots
                else 0.0,
                "final_nav": portfolio_snapshots[-1].nav
                if portfolio_snapshots
                else 1.0,
                "trading_summary": {
    ...
    def _signed_order_qty(self, order: Dict) -> float:
        qty = round(float(order.get("qty", 0)), 2) if order.get("qty") else 0.0
        side = str(order.get("side", "buy")).lower()
        if qty == 0:
            return 0.0
        return qty if side != "sell" else -abs(qty)

    def _fill_at_open(
        self, symbol: str, trade_date: pd.Timestamp, qty: float, open_price: float
    ) -> Tuple[float, float, float]:
        # Match at open price, considering slippage, commission and fill ratio
        logger.info(f"=== Cash flow calculation started [{symbol}] ===")
        logger.info(
            f"[CASH_FLOW] Initial params: symbol={symbol}, qty={qty}, open_price={open_price:.4f}"
        )

        side = 1 if qty > 0 else -1
        logger.info(
            f"[CASH_FLOW] Trade side: {'BUY' if side > 0 else 'SELL'} (side={side})"
        )

        # Compute slippage price for updating average position price
        px = (
            self.slippage.apply_buy(open_price)
            if side > 0
            else self.slippage.apply_sell(open_price)
        )
        logger.debug(
            f"[CASH_FLOW] Price after slippage: {px:.4f} (original: {open_price:.4f})"
        )

        planned_qty = abs(qty)
        filled_qty = round(
            planned_qty * max(0.0, min(1.0, self.fill_ratio)), 2
        )  # keep two decimals
        filled_qty = filled_qty * side
        logger.debug(
            f"[SHARES_CALCULATION] {symbol}: using open {open_price:.4f} (date: {trade_date})"
        )
        logger.debug(
            f"[SHARES_CALCULATION] {symbol}: planned_shares={planned_qty}, fill_ratio={self.fill_ratio}, filled_shares={filled_qty:.2f}"
        )

        # Use raw opening price to compute gross notional and net cost
        gross_open = open_price * abs(filled_qty)
        logger.debug(
            f"[CASH_FLOW] Gross notional (using open): {open_price:.4f} × {abs(filled_qty):.2f} = {gross_open:.2f}"
        )

        commission = self._apply_commission(gross_open)
        logger.debug(
            f"[CASH_FLOW] Commission: {gross_open:.2f} × {self.commission_bps / 10000:.4f} = {commission:.2f}"
        )
        ...
        if side > 0:
            logger.debug(
                f"[CASH_FLOW]   Buy net cost = gross + commission = {gross_open:.2f} + {commission:.2f} = {net_cost:.2f}"
            )
        else:
            logger.debug(
                f"[CASH_FLOW]   Sell net proceeds = -(gross - commission) = -({gross_open:.2f} - {commission:.2f}) = {net_cost:.2f}"
            )

        logger.info(
            f"[CASH_FLOW] Final: filled_qty={filled_qty:.2f}, exec_px={px:.4f} (for average cost), open_price={open_price:.4f} (for cash), net_cost={net_cost:.2f}"
        )
    ...
    def _apply_corporate_actions(
        self, pf: Portfolio, symbol: str, date: pd.Timestamp
    ) -> None:
                ...
                if date_col:
                    ...
                    for _, r in rows.iterrows():
                        ...
                        if ratio is None and "ratio" in df.columns:
                            try:
                                ratio = (
                                    float(r["ratio"]) if float(r["ratio"]) > 0 else None
                                )
                            except Exception:
                                ratio = None
                        if ratio and ratio > 0:
                            ...
                            _normalize_position_residual(pos)
                            logger.info(
                                f"[SHARES_CALCULATION] Split {symbol}: {old_shares:.2f} × {ratio} = {pos.shares:.2f} (date: {date})"
                            )
                ...
                if date_col and date_col in df.columns:
                    ...
                    for _, r in rows.iterrows():
                        ...
                        if cash and cash != 0.0:
                            ...
                            logger.info(
                                f"[DIVIDEND] Total dividend: {pos.shares:.2f} × {cash:.4f} = {dividend_amount:.2f}"
                            )
                            ...
                            if pf.update_cash(dividend_amount):
                                ...
                                logger.info(
                                    f"[DIVIDEND] Cash increased: +{dividend_amount:.2f}"
                                )
                            else:
    ...
    def _enforce_max_positions(self, pf: Portfolio, orders: List[Dict]) -> List[Dict]:
        ...
        buy_orders = [o for o in orders if self._signed_order_qty(o) > 0]
        sell_orders = [o for o in orders if self._signed_order_qty(o) <= 0]
    ...
    def run(
        ...
        self.run_id = run_id
        daily_output_dir = Path("storage/reports/backtest") / str(
            run_id or "latest_daily"
        )
        if hasattr(strategy, "set_daily_output_dir"):
            try:
                strategy.set_daily_output_dir(daily_output_dir)
            except Exception:
                pass

        # Original CLI --start / --end. Stays constant even under --resume,
        # so strategies (external agent warm-up cache keys, etc.) that need
        # a stable anchor for the *whole* window can rely on this instead
        # of the per-bar ctx["date"].
        ...
        pf = Portfolio(
            cash=float(self.cfg.get("backtest", {}).get("cash", portfolio_cash))
        )
        ...
        if resume_state is not None:
            pf = resume_state["portfolio"]
            nav = list(resume_state["nav"])
            trade_rows = list(resume_state.get("trade_rows", []))
            portfolio_snapshots = list(resume_state["portfolio_snapshots"])
            logger.info(
                f"[RESUME] Loaded snapshot from {resume_state['summary_path']} at {resume_state['resume_row'].get('trade_date')} with {len(pf.positions)} positions"
            )
            source_daily_dir = Path(resume_state["summary_path"]).parent
            if source_daily_dir.resolve() != daily_output_dir.resolve():
                daily_output_dir.mkdir(parents=True, exist_ok=True)
                logger.info(
                    f"[RESUME] Active daily artifact directory: {daily_output_dir}"
                )
            resume_cutoff = pd.Timestamp(
                str(
                    resume_state["resume_row"].get("trade_date")
                    or resume_state["resume_row"].get("date")
                )
            )
            dates = dates[dates > resume_cutoff]
        ...
        for i, d in enumerate(dates):
            ...
            if len(portfolio_snapshots) > 0:
                ...
                for symbol, symbol_info in last_snapshot.positions.items():
                    ...
                    if isinstance(symbol_info, dict):
                        mark_price = symbol_info.get("mark_price")
                    else:
                        mark_price = getattr(symbol_info, "mark_price", None)
                    if mark_price:
                        previous_open_map[symbol] = float(mark_price)
                logger.info(
                    f"[DEBUG] {d.strftime('%Y-%m-%d')} build previous_open_map: {len(previous_open_map)} symbols"
                )
                if previous_open_map:
                    for symbol, price in previous_open_map.items():
                        logger.info(
                            f"[DEBUG] {d.strftime('%Y-%m-%d')} previous_open_map[{symbol}] = {price}"
                        )
            ...
            for s in symbols:
                ...
                if not bars.empty:
                    try:
                        ...
                        logger.info(
                            f"[DEBUG] {d.strftime('%Y-%m-%d')} {s}: got open {open_px}, close {close_px}"
                        )
                        ...
                        logger.info(
                            f"[DEBUG] {d.strftime('%Y-%m-%d')} {s}: failed to parse prices - {e}"
                        )
                else:
                    open_px = None
                    close_px = None
                    symbols_without_data.append(s)
                    logger.info(
                        f"[DEBUG] {d.strftime('%Y-%m-%d')} {s}: daily bars empty (possibly holiday)"
                    )
                ...
                if open_px is not None:
                    ...
                    if s in previous_open_map:
                        ...
                        logger.info(
                            f"[DEBUG] {d.strftime('%Y-%m-%d')} {s}: using previous day's open as open {previous_open_map[s]}"
                        )
                    else:
                        logger.info(
                            f"[DEBUG] {d.strftime('%Y-%m-%d')} {s}: open_px is None, not added to open_map"
                        )

                # Use open price as mark price
                if open_px is not None:
                    mark_map[s] = open_px
                    close_map[s] = close_px if close_px is not None else open_px
                    logger.info(
                        f"[DEBUG] {d.strftime('%Y-%m-%d')} {s}: using open as mark price {open_px}"
                    )
                elif s in previous_open_map:
                    mark_map[s] = previous_open_map[s]
                    close_map[s] = previous_open_map[s]
                    logger.info(
                        f"[DEBUG] {d.strftime('%Y-%m-%d')} {s}: using previous day's open as mark price {previous_open_map[s]}"
                    )
            ...
            if not market_data_available:
                ...
                logger.info(
                    f"[HOLIDAY_DETECTED] Will use previous trading day's prices for valuation"
                )
                if previous_open_map:
                    logger.info(
                        f"[HOLIDAY_DETECTED] Available previous-day open prices: {len(previous_open_map)} symbols"
                    )
                else:
                    logger.info(
                        f"[HOLIDAY_DETECTED] Warning: no previous-day prices available"
                    )
                ...
                logger.info(
                    f"[PARTIAL_DATA] without data (will use previous day): {symbols_without_data}"
                )

            # Use unified fallback mechanism to further supplement prices
            from backtest.stockbench.core.price_utils import add_price_fallback_mechanism

            is_holiday = not market_data_available
            open_map = add_price_fallback_mechanism(
                open_map, previous_open_map, 100.0, is_holiday
            )
            mark_map = add_price_fallback_mechanism(
                mark_map, previous_open_map, 100.0, is_holiday
            )

            # Summary log: show final open_map state
            logger.info(
                f"[DEBUG] {d.strftime('%Y-%m-%d')} open_map built: {len(open_map)} symbols"
            )
            for sym, px in open_map.items():
                logger.info(f"[DEBUG] {d.strftime('%Y-%m-%d')} open_map[{sym}] = {px}")
            if not open_map:
                logger.info(
                    f"[DEBUG] {d.strftime('%Y-%m-%d')} Warning: open_map is empty!"
                )
            ...
            ref_price_map: Dict[str, float] = dict(
                open_map if self.rebalance_at_open else mark_map
            )
            ...
            ctx = {
                ...
                # Original CLI --start (stable across --resume). Strategies
                # that build warm-up windows should anchor on this, not on
                # the per-bar "date".
                ...
                "close_price_map": close_map,
                "previous_open_map": previous_open_map,
            ...
            # `agents.retry` is an alias for older llm_decision configs;
            # backtest.stockbench.order_retry is the strategy-agnostic setting.
            backtest_retry_cfg = (
                self.cfg.get("backtest", {}).get("order_retry", {})
                if self.cfg
                else {}
            )
            legacy_retry_cfg = self.cfg.get("agents", {}).get("retry", {}) if self.cfg else {}
            retry_cfg = backtest_retry_cfg or legacy_retry_cfg
            max_retries = int(
            ...
            while retry_attempt < max_retries:
                ...
                if rejected_orders:
                    ...
                    logger.info(
                        f"[RETRY_MECHANISM] Attempt {retry_attempt + 1}: Retrying with {len(rejected_orders)} rejected orders"
                    )
                else:
                ...
                total_sell_proceeds = 0  # Cash inflow from same-day sells (credits buys)
                order_costs = {}  # Cache to avoid recalculating

                for od in orders:
                    sym = od.get("symbol")
                    qty = self._signed_order_qty(od)
                    if sym and qty != 0 and open_map.get(sym) is not None:
                        try:
                            filled, exec_px, net_cost = self._fill_at_open(
                                sym, d, qty, float(open_map.get(sym))
                            )
                            order_costs[sym] = (filled, exec_px, net_cost)
                            if filled > 0 and net_cost > 0:  # Buy: cash outflow
                                total_cash_required += net_cost
                            elif filled < 0 and net_cost < 0:  # Sell: cash inflow
                                # net_cost is negative for sells; proceeds = -net_cost
                                total_sell_proceeds += -net_cost
                        except Exception as e:
                            logger.warning(
                                f"[PRECALC] {sym} cost calculation failed: {e}"
                            )
                            continue

                # Effective cash for buys = current cash + sells that execute today.
                # Without this credit, a rebalance (sell A → buy B) was incorrectly
                # rejected because the pre-check only saw ``pf.cash`` without the
                # inflow from the same-day sells that happen at the same open price.
                effective_cash = float(pf.cash) + total_sell_proceeds
                remaining_cash = effective_cash
                logger.info(
                    f"[PRECALC] Total cash required for all orders: {total_cash_required:.2f}, "
                    f"available cash: {pf.cash:.2f}, sell proceeds: {total_sell_proceeds:.2f}, "
                    f"effective cash: {effective_cash:.2f}"
                )
                ...
                for od in orders:
                    try:
                        ...
                        if sym in order_costs:
                            ...
                            filled, exec_px, net_cost = self._fill_at_open(
                                sym, d, qty, float(ref_px_i)
                            )

                        # Cash sufficiency check (cash-flow aware: sells credited via effective_cash)
                        if filled > 0 and net_cost > 0:  # only check buy trades
                            logger.info(
                                f"[DAY_PRECHECK_DEBUG] {sym}: remaining_cash={remaining_cash} (type: {type(remaining_cash)}), net_cost={net_cost} (type: {type(net_cost)})"
                            )
                            logger.info(
                                f"[DAY_PRECHECK_DEBUG] {sym}: remaining_cash < net_cost = {remaining_cash < net_cost}"
                            )
                            logger.info(
                                f"[DAY_PRECHECK_DEBUG] {sym}: remaining_cash >= net_cost = {remaining_cash >= net_cost}"
                            )

                            if remaining_cash < net_cost:
                                current_rejected_orders.append(
                                    {
                                        ...
                                        "context": {
                                            ...
                                            "sell_proceeds_today": total_sell_proceeds,
                                            "effective_cash_at_open": effective_cash,
                                            "remaining_cash_before_this_order": remaining_cash,
                                            "total_cash_required_all_orders": total_cash_required,
                                            "cash_shortfall": total_cash_required
                                            - effective_cash,
                                            "retry_attempt": retry_attempt,
                                            "all_orders_count": len(orders),
                                            "portfolio_rebalance_needed": True,
                                            "suggestion": f"Cumulative buy cost ({total_cash_required:.2f}) exceeds effective cash ({effective_cash:.2f}) by {total_cash_required - effective_cash:.2f} after crediting sell proceeds ({total_sell_proceeds:.2f}). Please reduce all order sizes proportionally or select fewer positions to fit within budget.",
                                        },
                                        "retry_count": retry_attempt,
                                    }
                                )
                                logger.info(
                                    f"[DAY_PRECHECK] {sym} precheck rejected: insufficient cash {remaining_cash:.2f} < required {net_cost:.2f} (effective cash {effective_cash:.2f} = pf.cash {pf.cash:.2f} + sells {total_sell_proceeds:.2f})"
                                )
                                logger.info(
                                    f"[DAY_PRECHECK] Total cash shortfall: {total_cash_required - effective_cash:.2f}"
                                )
                                continue
                            else:
                                logger.info(
                                    f"[DAY_PRECHECK] {sym} precheck passed: remaining_cash {remaining_cash:.2f} >= required {net_cost:.2f}"
                                )
                                remaining_cash -= net_cost

                        acceptable_orders.append(od)

                    except Exception as e:
                        logger.warning(
                            f"[DAY_PRECHECK] {sym if 'sym' in locals() else 'UNKNOWN'} precheck exception: {e}"
                        )
                        continue

                # Check if we should retry or proceed with execution
                if current_rejected_orders:
                    # Detect strategies that do not honor ctx["rejected_orders"]: when the
                    # rejected set is identical to the previous iteration's, further retries
                    # cannot make progress and only burn the full strategy pipeline again.
                    def _rejection_signature(orders_list):
                        return frozenset(
                            (o.get("symbol"), o.get("qty")) for o in orders_list or []
                        )
                    prev_signature = _rejection_signature(rejected_orders)
                    new_signature = _rejection_signature(current_rejected_orders)
                    rejected_orders = current_rejected_orders
                    if retry_attempt > 0 and new_signature == prev_signature:
                        logger.warning(
                            f"[RETRY_MECHANISM] Rejected orders unchanged across retries "
                            f"({len(current_rejected_orders)} orders); strategy is not "
                            f"consuming ctx['rejected_orders']. Short-circuiting at attempt "
                            f"{retry_attempt + 1}/{max_retries}; proceeding with "
                            f"{len(acceptable_orders)} acceptable orders."
                        )
                        break
                    retry_attempt += 1
                    if retry_attempt < max_retries:
                        logger.info(
                            f"[RETRY_MECHANISM] Detected {len(rejected_orders)} rejected orders; will retry (attempt {retry_attempt + 1}/{max_retries})"
                        )
                        continue  # Retry with rejected orders
                    else:
                        logger.warning(
                            f"[RETRY_MECHANISM] Max retries ({max_retries} from config) reached; proceeding with {len(acceptable_orders)} acceptable orders"
                        )
                        break  # No more retries, proceed with acceptable orders
                else:
                    logger.info(
                        f"[RETRY_MECHANISM] No rejected orders; proceeding with execution"
                    )
                    break  # No rejected orders, proceed with execution

            # Execute cash-inflow orders first so same-open rebalances can fund buys.
            def _execution_priority(order: Dict) -> int:
                sym = order.get("symbol")
                cost = order_costs.get(sym)
                if cost is None:
                    return 1
                return 0 if cost[2] < 0 else 1

            for od in sorted(acceptable_orders, key=_execution_priority):
                try:
                    sym = od.get("symbol")
                    qty = self._signed_order_qty(od)
                    if not sym or qty == 0:
                    ...
                    new_qty = self._maybe_redecide_qty(
                        strategy,
                        ctx,
                        sym,
                        pos_preview.shares,
                    ...
                    if isinstance(new_qty, int):
                        if new_qty == 0:
                            continue
                        filled, exec_px, net_cost = self._fill_at_open(
                            sym, d, new_qty, float(ref_px_i)
                        )
                    if filled != 0:
                        logger.info(f"\n=== DAY trade processing [{sym}] ===")
                        logger.info(f"[DAY_TRADE] trade date: {d.date()}")
                        logger.info(
                            f"[DAY_TRADE] params: filled={filled}, exec_px={exec_px:.4f}, net_cost={net_cost:.2f}"
                        )
                        ...
                        if filled > 0 and net_cost > 0:  # only check buys
                            if pf.cash < net_cost:
                                logger.info(
                                    f"[BACKTEST_CASH_CHECK] {sym} trade still rejected: insufficient cash {pf.cash:.2f} < required {net_cost:.2f}"
                                )
                                logger.info(
                                    f"[BACKTEST_CASH_CHECK] This should not happen; retry mechanism may have issues"
                                )
                                logger.info(
                                    f"=== DAY trade processing skipped [{sym}] ===\n"
                                )
                                continue

                    pos = self._ensure_position(pf, sym)
                    pre_trade_shares = float(pos.shares)
                    pre_trade_avg_price = float(pos.avg_price)
                    pre_trade_holding_days = int(pos.holding_days)
                    pre_trade_cash = float(pf.cash)

                    logger.info(f"[DAY_TRADE] Position update start")
                    logger.info(
                        f"[DAY_TRADE] Before: shares={pos.shares:.2f}, avg_price={pos.avg_price:.4f}"
                    )

                    old_shares = float(pos.shares)
                    old_avg_price = float(pos.avg_price)
                    old_total_cost = float(pos.total_cost)
                    new_shares = old_shares + filled
                    logger.info(
                        f"[DAY_TRADE] New shares: {old_shares:.2f} + {filled:.2f} = {new_shares:.2f}"
                    )

                    next_shares = new_shares
                    next_avg_price = old_avg_price
                    next_total_cost = old_total_cost

                    if old_shares == 0 or old_shares * filled > 0:
                        if new_shares != 0:
                            next_avg_price = (
                                old_avg_price * abs(old_shares) + abs(filled) * exec_px
                            ) / abs(new_shares)
                        next_total_cost = old_total_cost + abs(net_cost)
                        logger.info(
                            f"[DAY_TRADE] Same-direction total cost: {old_total_cost:.2f} + {abs(net_cost):.2f} = {next_total_cost:.2f}"
                        )
                    elif abs(filled) < abs(old_shares):
                        next_avg_price = old_avg_price
                        cost_ratio = abs(filled) / abs(old_shares)
                        cost_reduction = old_total_cost * cost_ratio
                        next_total_cost = old_total_cost - cost_reduction
                        logger.info(
                            f"[DAY_TRADE] Partial close cost reduction: {cost_ratio:.4f} × cost = {cost_reduction:.2f}, new cost: {next_total_cost:.2f}"
                        )
                    elif abs(filled) == abs(old_shares):
                        next_avg_price = 0.0
                        next_total_cost = 0.0
                        ...
                        next_avg_price = exec_px
                        next_total_cost = abs(new_shares) * exec_px
                        logger.info(
                            f"[DAY_TRADE] Reverse trade residual cost: {abs(new_shares):.2f} × {exec_px:.4f} = {next_total_cost:.2f}"
                        )

                    # Safe cash update (cash already checked) before committing position state.
                    logger.info(f"[DAY_TRADE] Update cash by net cost: -{net_cost:.2f}")
                    if not pf.update_cash(-net_cost):
                        logger.warning(
                            f"[CASH_PROTECTION] Unexpected: cash update failed, cannot pay {net_cost:.2f}"
                        )
                        logger.info(
                            f"[CASH_PROTECTION] This should not happen; cash was pre-checked"
                        )
                        continue

                    pos.shares = next_shares
                    pos.avg_price = next_avg_price if next_shares != 0 else 0.0
                    pos.total_cost = next_total_cost if next_shares != 0 else 0.0
                    _normalize_position_residual(pos)

                    logger.info(
                        f"[DAY_TRADE] After: shares={pos.shares:.2f}, avg_price={pos.avg_price:.4f}"
                    )
                    ...
                    trade_record = self._create_trade_record(
                        ...
                        open_map,
                        d,
                        pre_trade_shares=pre_trade_shares,
                        pre_trade_avg_price=pre_trade_avg_price,
                        pre_trade_holding_days=pre_trade_holding_days,
                    ...
                    trade_rows.append(
                        {
                            ...
                            "exec_ref_price": float(
                                ref_px_i
                            ),  # Opening price (used for cash update)
                            "commission_bps": self.commission_bps,
                            "fill_ratio": self.fill_ratio,
                            "trade_value": abs(filled)
                            * float(
                                ref_px_i
                            ),  # Trade amount calculated using opening price
                        }
                    )
                    ...
                    if sym not in executed_symbols:
                        ...
                        logger.info(
                            f"[DELAYED_RECORD] Daily trade successful, marking symbol: {sym}"
                        )
                except Exception as e:
            ...
            for s, pos in pf.positions.items():
                ...
                logger.info(
                    f"[DAILY_NAV]   {s}: {pos.shares:.2f} × {px:.4f} = {position_value:.2f}"
                )
                if pos.shares > 0:
                    pos.holding_days += 1
            ...
            logger.info(
                f"[DAILY_NAV] Total position market value: {mark_to_market:.2f}"
            )
            logger.info(
                f"[DAILY_NAV] Total equity: {pf.cash:.2f} + {mark_to_market:.2f} = {total_equity:.2f}"
            )
            logger.info(f"[DAILY_NAV] Initial capital: {initial_cash:.2f}")
            logger.info(
                f"[DAILY_NAV] Daily NAV: {total_equity:.2f} / {initial_cash:.2f} = {daily_nav:.6f}"
            )
            ...
            if hasattr(strategy, "record_executed_decisions"):
                if executed_symbols:
                    logger.info(
                        f"[DELAYED_RECORD] Daily trading completed, recording executed decisions"
                    )
                else:
                    logger.info(
                        f"[DELAYED_RECORD] No successful trades today, but still need to record hold decisions"
                    )
                strategy.record_executed_decisions(executed_symbols, portfolio=pf)

            # Daily summary, after_bar, memory, and resume state must share the
            # same mark-to-market basis as the NAV just calculated above.
            portfolio_snapshot = self._create_portfolio_snapshot(
                pf, d, open_map, previous_open_prices=previous_open_map
            )
            portfolio_snapshots.append(portfolio_snapshot)

            after_bar_update = {}
            if hasattr(strategy, "after_bar"):
                try:
                    update = strategy.after_bar(
                        {
                            "date": d,
                            "symbols": symbols,
                            "portfolio": pf,
                            "cash": float(pf.cash),
                            "mark_to_market": round(float(mark_to_market), 6),
                            "total_equity": round(float(total_equity), 6),
                            "daily_nav": round(float(daily_nav), 8),
                            "executed_symbols": list(executed_symbols),
                            "orders_submitted": len(orders),
                            "orders_accepted": len(acceptable_orders),
                            "orders_rejected": len(rejected_orders),
                            "portfolio_snapshot": portfolio_snapshot.__dict__,
                            "daily_output_dir": str(daily_output_dir),
                        }
                    )
                    if isinstance(update, dict):
                        after_bar_update = update
                except Exception:
                    logger.exception("[AFTER_BAR] strategy.after_bar failed")
            ...
            if run_id:
                ...
                if hasattr(strategy, "set_daily_output_dir"):
                    try:
                        strategy.set_daily_output_dir(output_dir)
                    except Exception:
                        pass
                daily_summary_path = Path(output_dir) / "daily_run_summary.jsonl"
                with daily_summary_path.open("a", encoding="utf-8") as fh:
                    fh.write(
                        json.dumps(
                            {
                                "trade_date": d.strftime("%Y-%m-%d"),
                                "cash": round(float(pf.cash), 6),
                                "mark_to_market": round(float(mark_to_market), 6),
                                "total_equity": round(float(total_equity), 6),
                                "daily_nav": round(float(daily_nav), 8),
                                "executed_symbols": list(executed_symbols),
                                ...
                                "position_count": len(
                                    [
                                        ...
                                        if abs(float(pos.shares)) > _POSITION_EPSILON
                                    ]
                                ),
                                "portfolio_snapshot": portfolio_snapshot.__dict__,
                                **after_bar_update,
        ...
        try:
            ...
            if bench_cfg and isinstance(bench_cfg, dict):
                ...
                if type_str == "per_symbol_buy_and_hold":
                    ...
                    trade_price_field = str(
                        bench_cfg.get("trade_price_field", "open")
                    ).lower()
                    ...
                    try:
                        ...
                        fill_limit_bh = (
                            int(fill_limit_raw) if fill_limit_raw is not None else 0
                        )
                    except Exception:
                        fill_limit_bh = 0
                    if (
                        isinstance(per_symbol_nav_raw, pd.DataFrame)
                        and not per_symbol_nav_raw.empty
                    ):
                        ...
                        if fill_mode == "ffill":
                            lim = (
                                None
                                if int(fill_limit_bh or 0) <= 0
                                else int(fill_limit_bh)
                            )
                        ...
                        sortino_mode = str(
                            (
                                (bench_cfg.get("daily_metrics", {}) or {}).get(
                                    "sortino", {}
                                )
                                or {}
                            ).get("mode", "rolling")
                        ...
                        for sym in ps.columns:
                            try:
                                ...
                                try:
                                    ...
                                    metrics_list = None
                                met = compute_nav_to_metrics_series(
                                    ps[sym].dropna(),
                    ...
                    # Continue execution, allowing single/basket benchmarks simultaneously
                reindex_mode = str(bench_cfg.get("reindex", "inner_join")).lower()
                fill_mode = str(bench_cfg.get("fill", "ffill")).lower()
                fill_limit_raw = bench_cfg.get("fill_limit", 0)
                try:
                    ...
                    fill_limit = 0
                # Daily benchmark
                series_dict = load_benchmark_components(
                    bench_cfg, self.datasets, nav_df.index, field="adjusted_close"
                )
                if series_dict:
                    ret_df = price_to_returns(series_dict)
                    if not ret_df.empty:
                        if (
                            isinstance(bench_cfg.get("basket"), list)
                            and len(bench_cfg.get("basket", [])) > 0
                        ):
                            reb = str(bench_cfg.get("rebalance", "daily")).lower()
                            r_b = aggregate_with_rebalance(
                                ret_df, bench_cfg.get("basket", []), freq=reb
                            )
                            benchmark_nav = (1.0 + r_b).cumprod()
                        else:
                            # Single benchmark: normalize by price
                            price = (
                                list(series_dict.values())[0].reindex(ret_df.index).ffill()
                            )
                            benchmark_nav = price / max(price.iloc[0], 1e-12)
                        # Align to strategy NAV index
                        benchmark_nav = align_with_strategy_nav(
                            benchmark_nav,
                            nav_df.index,
