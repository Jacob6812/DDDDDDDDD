...
from backtest.stockbench.utils.io import ensure_dir
...
def _as_series(
    obj: Any, preferred_names: tuple[str, ...] = ("nav",)
) -> pd.Series | None:
...
def write_outputs(
    result: Dict, run_id: str | None = None, cfg: Dict[str, Any] | None = None
) -> str:
    ...
    try:
        ...
        summary_row = {
            ...
            # Relative metrics
            "information_ratio_daily": float(
                metrics.get("information_ratio_daily", 0.0)
            ),
        ...
        pd.DataFrame([summary_row]).to_csv(
            os.path.join(base, "metrics_summary.csv"), index=False
        )
    except Exception:
        pass
    ...
    try:
        ...
        if (
            nav_s is not None
            and bench_s is not None
            and len(nav_s) > 0
            and len(bench_s) > 0
        ):
            df_plot = pd.concat(
                [nav_s.rename("strategy"), bench_s.rename("benchmark")], axis=1
            ).dropna()
            if len(df_plot) > 0:
    ...
    # Prioritize NAV-based period (can display even without trades), otherwise fallback to trades
    try:
    ...
    try:
        ...
        timespan = (
            (cfg or {}).get("backtest", {}).get("timespan")
            if isinstance(cfg, dict)
            else None
        )
        agent_mode = (
            (cfg or {}).get("agents", {}).get("mode") if isinstance(cfg, dict) else None
        )
    ...
    try:
        bench_cfg = (
            (cfg or {}).get("backtest", {}).get("benchmark", {})
            if isinstance(cfg, dict)
            else {}
        )
        if bench_cfg:
            if bench_cfg.get("basket"):
                comp = ",".join(
                    [
                        it.get("symbol") if isinstance(it, dict) else str(it)
                        for it in bench_cfg.get("basket", [])
                    ]
                )
                summary_lines.append(
    ...
    try:
        if (
            "information_ratio" in metrics
            or "tracking_error" in metrics
            or "alpha_simple" in metrics
        ):
            summary_lines.append(
                "relative: "
    ...
    # Solidify baseline (optional): write to baselines directory when cfg.backtest.stockbench.baseline_name exists
    try:
        baseline_name = None
        if isinstance(cfg, dict):
        ...
        if baseline_name:
            bdir = os.path.join(
                os.getcwd(), "storage", "reports", "backtest", "baselines"
            )
            ensure_dir(bdir)
            payload = {
                "run_id": run_id,
                ...
                "params": {
                    "commission_bps": (cfg or {})
                    .get("backtest", {})
                    .get("commission_bps")
                    if isinstance(cfg, dict)
                    else None,
                    "slippage_bps": (cfg or {}).get("backtest", {}).get("slippage_bps")
                    if isinstance(cfg, dict)
                    else None,
                    "fill_ratio": (cfg or {}).get("backtest", {}).get("fill_ratio")
                    if isinstance(cfg, dict)
                    else None,
                    "news_agg": (cfg or {}).get("news", {}).get("agg")
                    if isinstance(cfg, dict)
                    else None,
                    "trim_alpha": (cfg or {}).get("news", {}).get("trim_alpha")
                    if isinstance(cfg, dict)
                    else None,
                },
            }
            with open(
                os.path.join(bdir, f"{baseline_name}.json"), "w", encoding="utf-8"
            ) as f:
                json.dump(payload, f, ensure_ascii=False, indent=2)
    except Exception:
        pass

    # ===== per-symbol buy&hold benchmark outputs =====
    ...
    try:
        ...
        if per_symbol_nav is not None or per_symbol_metrics is not None:
                ...
                try:
                    ...
                    if frames:
                        ...
                        met_path = os.path.join(
                            ps_dir, "per_symbol_benchmark_metrics.parquet"
                        )
                        try:
                            big.to_parquet(met_path, index=False)
                        except Exception:
                            big.to_csv(
                                met_path.replace(".parquet", ".csv"), index=False
                            )
                        # Also save as jsonl (grouped by symbol for easy reading)
                        jsonl_path = os.path.join(
                            ps_dir, "per_symbol_benchmark_metrics.jsonl"
                        )
                        with open(jsonl_path, "w", encoding="utf-8") as jf:
                            for sym, df in per_symbol_metrics.items():
                                payload = {
                                    "symbol": sym,
                                    "metrics": _json_safe(df.to_dict(orient="index")),
                                }
                    ...
                    for sym, df in per_symbol_metrics.items():
                        try:
                            ...
                            metrics_cfg = (
                                ...
                                else ["cum_return", "max_drawdown", "sortino"]
                            )
                            ...
                            with open(
                                os.path.join(ps_dir, f"{sym}_metrics.txt"),
                                "w",
                                encoding="utf-8",
                            ) as ftxt:
                                ftxt.write("\n".join(txt))
                    ...
                    try:
                        ...
                        for sym in list(per_symbol_nav.columns):
                            try:
                                ...
                                draw_cum = (not metrics_cfg) or (
                                    "cum_return" in metrics_cfg
                                )
                                ...
                                try:
                                    ...
                                    cbps = float(
                                        bt_cfg.get("commission_bps", 0.0) or 0.0
                                    )
                                    ...
                                    fee_str = (
                                        ",".join(fee_note) if fee_note else "no-fee"
                                    )
            ...
            try:
                from backtest.stockbench.visualization import (
                ...
                aggregated_cfg = (
                    ...
                    .get("backtest", {})
                    .get("benchmark", {})
                    .get("aggregated_analysis", {})
                )
                if aggregated_cfg.get("enabled", True) and hasattr(
                    per_symbol_nav, "columns"
                ):
                    plots_cfg = aggregated_cfg.get("plots", {})

                    # 1. Cumulative return aggregated analysis chart
                    if plots_cfg.get("cumreturn_analysis", True):
                        cumret_path = os.path.join(
                            ps_dir, "aggregated_cumreturn_analysis.png"
                        )
                        plot_aggregated_cumreturn_analysis(
                            per_symbol_nav, cumret_path, aggregated_cfg
                        )
                    ...
                    if plots_cfg.get("price_trends", True):
                        ...
                        plot_stock_price_trends(
                            per_symbol_nav, trends_path, aggregated_cfg
                        )
                    ...
                    existing_stock_images = glob.glob(
                        os.path.join(ps_dir, "*_metrics.png")
                    )
                    for img_path in existing_stock_images:
                        ...
                        if filename not in [
                            "aggregated_cumreturn_analysis.png",
                    ...
                    generate_individual_stocks_summary(
                        per_symbol_nav, individual_dir, cfg
                    )
                ...
                multi_period_cfg = (
                    ...
                    .get("backtest", {})
                    .get("benchmark", {})
                    .get("multi_period_analysis", {})
                )
                if multi_period_cfg.get("enabled", True) and hasattr(
                    per_symbol_nav, "columns"
                ):
                    try:
                        from backtest.stockbench.visualization import (
                        ...
                        if plots_cfg.get("performance_heatmap", True):
                            heatmap_path = os.path.join(
                                ps_dir, "multi_period_performance_heatmap.png"
                            )
                            plot_multi_period_performance_heatmap(
                                per_symbol_nav,
                                heatmap_path,
                                dpi=dpi,
                                cfg=multi_period_cfg,
                            )

                        # 2. Rolling Sortino ratio comparison
                        if plots_cfg.get("rolling_sortino", True):
                            window = windows_cfg.get("rolling_window", 63)
                            sortino_path = os.path.join(
                                ps_dir, "rolling_sortino_comparison.png"
                            )
                            plot_rolling_metrics_comparison(
                                per_symbol_nav,
                                sortino_path,
                        ...
                        if plots_cfg.get("rolling_sharpe", True):
                            ...
                            sharpe_path = os.path.join(
                                ps_dir, "rolling_sharpe_comparison.png"
                            )
                            plot_rolling_metrics_comparison(
                                per_symbol_nav,
                                sharpe_path,
                        ...
                        if plots_cfg.get("rolling_drawdown", True):
                            ...
                            drawdown_path = os.path.join(
                                ps_dir, "rolling_drawdown_comparison.png"
                            )
                            plot_rolling_metrics_comparison(
                                per_symbol_nav,
                                drawdown_path,
                        ...
                        if plots_cfg.get("ranking_over_time", True):
                            ...
                            ranking_path = os.path.join(
                                ps_dir, "performance_ranking_over_time.png"
                            )
                            plot_performance_ranking_over_time(
                                per_symbol_nav,
                                ranking_path,
                        ...
                        print(
                            f"[WARNING] Failed to import multi-period visualization module: {e}"
                        )
                    except Exception as e:
                        print(
                            f"[WARNING] Failed to generate multi-period analysis: {e}"
                        )

                # ===== New: Benchmark comparison image generation =====
                try:
                    # Check if benchmark comparison is enabled
                    benchmark_comparison_cfg = aggregated_cfg.get(
                        "benchmark_comparison", {}
                    )
                    if benchmark_comparison_cfg.get("enabled", True) and hasattr(
                        per_symbol_nav, "columns"
                    ):
                        from backtest.stockbench.visualization import (
                            plot_nav_comparison,
                        ...
                        from backtest.stockbench.metrics import (
                        ...
                        initial_cash = result.get(
                            "initial_cash", 1000000
                        )  # Default 1 million

                        if strategy_nav is not None and not per_symbol_nav.empty:
                            # Calculate benchmarks
                            simple_avg_nav = compute_simple_average_benchmark(
                                per_symbol_nav
                            )
                            weighted_avg_nav = compute_weighted_average_benchmark(
                                per_symbol_nav
                            )
                            ...
                            if not simple_avg_nav.empty:
                                simple_dir = os.path.join(
                                    comparison_dir, "strategy_vs_simple_avg"
                                )
                                os.makedirs(simple_dir, exist_ok=True)

                                nav_path = os.path.join(
                                    simple_dir, "nav_comparison.png"
                                )
                                plot_nav_comparison(
                                    strategy_nav,
                                    simple_avg_nav,
                                ...
                                assets_path = os.path.join(
                                    simple_dir, "totalassets_comparison.png"
                                )
                                plot_totalassets_comparison(
                                    strategy_nav,
                                    simple_avg_nav,
                            ...
                            if not weighted_avg_nav.empty:
                                weighted_dir = os.path.join(
                                    comparison_dir, "strategy_vs_weighted_avg"
                                )
                                os.makedirs(weighted_dir, exist_ok=True)

                                nav_path = os.path.join(
                                    weighted_dir, "nav_comparison.png"
                                )
                                plot_nav_comparison(
                                    strategy_nav,
                                    weighted_avg_nav,
                                ...
                                assets_path = os.path.join(
                                    weighted_dir, "totalassets_comparison.png"
                                )
                                plot_totalassets_comparison(
                                    strategy_nav,
                                    weighted_avg_nav,
                            ...
                            if spy_nav is not None and not spy_nav.empty:
                                spy_dir = os.path.join(
                                    comparison_dir, "strategy_vs_spy"
                                )
                                ...
                                assets_path = os.path.join(
                                    spy_dir, "totalassets_comparison.png"
                                )
                                plot_totalassets_comparison(
                                    strategy_nav,
                                    spy_nav,
                        ...
                        print(
                            f"[OK] Generated benchmark comparison plots in {comparison_dir}"
                        )

                except ImportError as e:
                    print(
                        f"[WARNING] Failed to import benchmark comparison module: {e}"
                    )
                except Exception as e:
