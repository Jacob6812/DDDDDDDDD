...
def plot_aggregated_cumreturn_analysis(
    per_symbol_nav_df: pd.DataFrame, output_path: str, cfg: Optional[Dict] = None
) -> None:
    ...
    try:
        ...
        if len(simple_avg_nav) > 0:
            ...
            ax.plot(
                simple_avg_cumret.index,
                simple_avg_cumret.values,
                color="#FF4444",
                linewidth=linewidth,
        ...
        if len(weighted_avg_nav) > 0:
            base_nav = (
                weighted_avg_nav.iloc[0] if weighted_avg_nav.iloc[0] != 0 else 1.0
            )
            weighted_avg_cumret = weighted_avg_nav / base_nav - 1.0
            ax.plot(
                weighted_avg_cumret.index,
                weighted_avg_cumret.values,
                color="#4444FF",
                linewidth=linewidth,
    ...
    ax.set_title(
        "Multi-Stock Cumulative Return Analysis", fontsize=14, fontweight="bold"
    )
    ...
    try:
        ...
        if key_metrics_cfg.get("annotation_box", True):
            ...
            simple_avg_nav = compute_simple_average_benchmark(per_symbol_nav_df)
            metrics_summary = extract_key_metrics_summary(
                simple_avg_nav, highlight_metrics
            )
    ...
    try:
        ...
        plt.close(fig)
        print(f"[OK] Saved aggregated cumreturn analysis to {output_path}")
    except Exception as e:
...
def plot_stock_price_trends(
    ...
    try:
        ...
        if len(simple_avg_nav) > 0:
            ...
            ax1.plot(
                simple_avg_normalized.index,
                simple_avg_normalized.values,
                color="#FF4444",
                linewidth=linewidth,
                label="Equal Weight Average",
                linestyle="-",
    ...
    ax1.set_title(
        "Stock Price Trend Comparison (Normalized)", fontsize=14, fontweight="bold"
    )
    ...
    try:
        ...
        plt.savefig(output_path, dpi=dpi, bbox_inches="tight")
        plt.close(fig)
        print(f"[OK] Saved stock price trends to {output_path}")
    except Exception as e:
        print(f"[ERROR] Failed to save trends plot to {output_path}: {e}")
        plt.close(fig)


def generate_individual_stocks_summary(
    per_symbol_nav_df: pd.DataFrame, output_dir: str, cfg: Optional[Dict] = None
) -> None:
    ...
    try:
        ...
        summary_lines.append(
            f"Number of stocks analyzed: {len(per_symbol_nav_df.columns)}"
        )
        summary_lines.append(
            f"Analysis period: {per_symbol_nav_df.index[0].date()} to {per_symbol_nav_df.index[-1].date()}"
        )
        ...
        summary_lines.append(
            "- This directory contains detailed charts for each stock (if image format saving is enabled)"
        )
        summary_lines.append(
            "- Detailed metrics files for each stock (if text format saving is enabled)"
        )
        summary_lines.append(
            "- All data is based on buy&hold strategy, considering configured transaction costs"
        )
        ...
        print(f"[OK] Generated individual stocks summary at {summary_path}")
...
def plot_nav_comparison(
    ...
    # Align data
    df = pd.concat(
        [strategy_nav.rename("strategy"), benchmark_nav.rename("benchmark")], axis=1
    ).dropna()
    ...
    ax1.plot(
        df.index, df["benchmark"], linewidth=2, label=benchmark_label, color="orange"
    )
    ax1.set_ylabel("NAV", fontsize=12)
    ax1.set_title(
        f"{strategy_label} vs {benchmark_label} - NAV Comparison",
        fontsize=14,
        fontweight="bold",
    )
    ax1.legend()
    ...
    ax2.fill_between(
        ...
        alpha=0.3,
        color="blue",
        label=f"{strategy_label} Drawdown",
    )
    ax2.fill_between(
        df.index,
        benchmark_dd,
        ...
        alpha=0.3,
        color="orange",
        label=f"{benchmark_label} Drawdown",
    )
    ...
    try:
        ...
        plt.savefig(output_path, dpi=dpi, bbox_inches="tight")
        plt.close(fig)
        print(f"[OK] Saved nav comparison to {output_path}")
    except Exception as e:
        print(f"[ERROR] Failed to save comparison plot to {output_path}: {e}")
        plt.close(fig)


def plot_totalassets_comparison(
    strategy_nav: pd.Series,
    benchmark_nav: pd.Series,
    strategy_label: str,
    benchmark_label: str,
    ...
    # Align data
    df = pd.concat(
        [strategy_assets.rename("strategy"), benchmark_assets.rename("benchmark")],
        axis=1,
    ).dropna()

    if df.empty:
        print(
            f"[WARNING] No aligned data for totalassets comparison, skipping {output_path}"
        )
        plt.close(fig)
        return
    ...
    ax.plot(
        ...
        df["benchmark"],
        linewidth=2,
        label=f"{benchmark_label} Assets",
        color="orange",
    )
    ...
    ax.set_xlabel("Date", fontsize=12)
    ax.set_title(
        f"{strategy_label} vs {benchmark_label} - Total Assets Comparison",
        fontsize=14,
        fontweight="bold",
    )
    ax.legend()
    ...
    ax.yaxis.set_major_formatter(
        ticker.FuncFormatter(lambda x, p: f"${x / 1000000:.1f}M")
    )
    ...
    ax.text(
        0.02,
        0.95,
        f"{strategy_label}: ${final_strategy:,.0f}",
        transform=ax.transAxes,
        fontsize=12,
    ...
    ax.text(
        0.02,
        0.88,
        f"{benchmark_label}: ${final_benchmark:,.0f}",
        transform=ax.transAxes,
        fontsize=12,
    ...
    ax.text(
        0.02,
        0.81,
        f"Difference: ${final_strategy - final_benchmark:,.0f}",
        transform=ax.transAxes,
        fontsize=12,
    ...
    try:
        ...
        plt.savefig(output_path, dpi=dpi, bbox_inches="tight")
        plt.close(fig)
        print(f"[OK] Saved total assets comparison to {output_path}")
    except Exception as e:
...
def plot_multi_period_performance_heatmap(
    per_symbol_nav_df, output_path, dpi=300, cfg=None
):
    """
    ...
    try:
        ...
        default_windows = [
            ...
            126,
            252,
        ]  # week, 2-week, month, 2-month, quarter, half-year, year
        ...
        try:
            ...
            print(
                f"[WARNING] Failed to parse heatmap_periods config, using defaults: {e}"
            )
            windows = default_windows
        ...
        for symbol in symbols:
            ...
            for window in windows:
                if len(nav_series) >= window:
                    ...
                    latest_return = (
                        rolling_returns.iloc[-1] if not rolling_returns.empty else 0
                    )
        ...
        ax.set_title(
            "Multi-Period Return Heatmap (%)", fontsize=14, fontweight="bold", pad=20
        )
        ...
        for i in range(len(symbols)):
            for j in range(len(window_labels)):
                if not np.isnan(performance_matrix[i, j]):
                    ...
                    color = (
                        "white"
                        if abs(performance_matrix[i, j])
                        > np.nanmax(np.abs(performance_matrix)) * 0.6
                        else "black"
                    )
                    ax.text(
        ...
        print(f"[OK] Saved multi-period performance heatmap to {output_path}")
...
def plot_rolling_metrics_comparison(
    per_symbol_nav_df, output_path, metric="sortino", window=63, dpi=300
):
    """
    ...
    try:
        ...
        for i, symbol in enumerate(symbols):
            ...
            if metric == "sortino":
                metrics_df = compute_nav_to_metrics_series(
                    nav_series, sortino_mode="rolling", window=window
                )
        ...
        print(f"[OK] Saved rolling {metric} comparison to {output_path}")
...
def plot_performance_ranking_over_time(
    per_symbol_nav_df, output_path, window=21, dpi=300
):
    """
    ...
    try:
        ...
        # Set chart properties
        ax.set_title(
            f"Stock Return Ranking Over Time ({window}-day rolling)",
            fontsize=14,
            fontweight="bold",
            pad=20,
        )
        ...
        print(f"[OK] Saved performance ranking chart to {output_path}")
