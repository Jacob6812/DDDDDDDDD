...
from backtest.stockbench.utils.logging_helper import get_llm_logger
...
class LLMConfig:
    provider: str = (
        "openai"  # Change default provider to openai, use OpenAI SDK uniformly
    )
    ...
    # Granular read/write control (defaults to cache_enabled when not set)
...
class LLMClient:
    def __init__(
        self, api_key_env: str = "OPENAI_API_KEY", cache_dir: Optional[str] = None
    ) -> None:
        self.api_key = os.getenv(api_key_env) or os.getenv("LLM_API_KEY", "")
        self.cache_dir = cache_dir or os.path.join(
            os.getcwd(), "storage", "cache", "llm"
        )
    ...
    def _get_openai_client(self, cfg: LLMConfig) -> openai.OpenAI:
        ...
        if self._openai_client is None:
            self._openai_client = openai.OpenAI(
                api_key=self.api_key, base_url=cfg.base_url, timeout=cfg.timeout_sec
            )
    ...
    def _cache_path(
        self, key: str, run_id: Optional[str] = None, role: Optional[str] = None
    ) -> str:
    ...
    def _read_cache(
        self, key: str, ttl_hours: int, run_id: Optional[str] = None
    ...
    def _make_cache_key(
        self, role: str, cfg: LLMConfig, system_prompt: str, user_prompt: str
    ) -> str:
        ident = {
            "role": role,
            ...
            try:
                ...
                if json_str:
                    ...
                    self.llm_logger.debug(
                        f"🎯 JSON extraction successful - Strategy {i + 1}"
                    )
                    return parsed_json
            except (json.JSONDecodeError, AttributeError, TypeError) as e:
                self.llm_logger.debug(
                    f"⚠️ JSON extraction failed - Strategy {i + 1}: {str(e)[:50]}"
                )
                continue
    ...
    def _handle_truncated_response(self, content: str) -> str:
        ...
        try:
            ...
            for symbol, action in basic_matches:
                ...
                if not reasons:
                    if action == "hold":
                        ...
                        reasons = [
                            f"{action} decision extracted from truncated response",
                            "Based on partial available data",
                        ]
            ...
            if stock_decisions:
                ...
                json.dumps(
                    result_json
                )  # This will throw exception if format is incorrect
                self.llm_logger.info(
                    f"✅ Successfully extracted {len(stock_decisions)} stock decisions from truncated response"
                )
    ...
    def _ensure_decisions_extracted(
        self,
        ...
        self.llm_logger.info(
            f"Starting recursive parsing for raw_content, max attempts: {max_attempts}"
        )
            ...
            try:
                ...
                if not content:
                    self.llm_logger.warning(
                        f"No content found for parsing attempt {attempt + 1}"
                    )
                    break
                ...
                if extracted and isinstance(extracted, dict):
                    if "decisions" in extracted:
                        self.llm_logger.info(
                            f"Successfully extracted decisions on attempt {attempt + 1}"
                        )
                        return extracted
                    else:
                        self.llm_logger.debug(
                            f"Attempt {attempt + 1}: Extracted JSON but no decisions found"
                        )
                        # Try to find nested decisions in extracted result
                        decisions = self._find_nested_decisions(extracted)
                        if decisions:
                            self.llm_logger.info(
                                f"Found nested decisions on attempt {attempt + 1}"
                            )
                            return {"decisions": decisions}
                else:
                    self.llm_logger.debug(
                        f"Attempt {attempt + 1}: Failed to extract valid JSON"
                    )
        ...
        self.llm_logger.warning(
            f"Failed to extract decisions after {max_attempts} attempts, returning raw_content"
        )
        return parsed_response
        ...
        try:
            ...
            enhanced_cache_data = {
                ...
                "output": {
                    ...
                    "raw_response_length": len(str(raw_response))
                    if raw_response
                    else 0,
                },
            }
    ...
    def get_cached_payload(
        self, cache_key: str, run_id: Optional[str] = None, role: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
            ...
            with open(path, "r", encoding="utf-8") as f:
                ...
                if (
                    ...
                    if (
                        ...
                        and "decisions" in parsed_response
                    ):
                    ...
                    if (
                        isinstance(parsed_response, dict)
                        and "raw_content" in parsed_response
                    ):
                        self.llm_logger.debug(
                            "Found raw_content in cache, attempting to reparse from raw_response"
                        )
                        ...
                        if isinstance(raw_response, dict) and "choices" in raw_response:
                            try:
                                ...
                                if content:
                                    ...
                                    reparsed = self._extract_json_with_improved_logic(
                                        content
                                    )
                                    if reparsed and "decisions" in reparsed:
                                        self.llm_logger.debug(
                                            "Successfully reparsed decisions from raw_response"
                                        )
                                        return reparsed
                            except Exception as e:
                                self.llm_logger.warning(
                                    f"Failed to reparse from raw_response: {e}"
                                )

                    # Before returning, if result is still raw_content, try recursive parsing
                    raw_response = cached_data["output"].get("raw_response", {})
                    final_result = self._ensure_decisions_extracted(
    ...
    def generate_json(
        ...
        self.llm_logger.info(f"[LLM] {role} - Model: {cfg.model}")

        # Check cache
        cache_read = (
            cfg.cache_read_enabled
            if cfg.cache_read_enabled is not None
            else cfg.cache_enabled
        )
        if cache_read and self.cache_dir:
            # Prioritize using date-included cache key
            cache_key = self._make_cache_key_with_date(
                role, cfg, system_prompt, user_prompt, trade_date, retry_attempt
            )
            cached = self.get_cached_payload(cache_key, run_id, role)
            if cached:
                meta["cached"] = True
                self.llm_logger.info(f"[CACHE] Cache hit - {role}")
                return cached, meta
            else:
                self.llm_logger.debug(f"[CACHE] Cache miss - {role}")

        provider = (cfg.provider or "openai-compatible").lower()
        need_auth = (
            cfg.auth_required
            if cfg.auth_required is not None
            else (
                provider
                not in {"vllm", "openai-compatible-no-auth", "llama.cpp", "none"}
            )
        )

        # Record authentication info
        self.llm_logger.debug(f"[AUTH] Provider: {provider}, Needs auth: {need_auth}")

        headers: Dict[str, str] = {"Content-Type": "application/json"}
        if need_auth:
            if self.api_key:
                headers["Authorization"] = f"Bearer {self.api_key}"
                self.llm_logger.debug("[AUTH] Added authentication header")
            else:
                self.llm_logger.error(
                    f"[ERROR] Authentication required but no API key provided - {provider}"
                )
        ...
        self.llm_logger.debug(f"[REQ] Request preparation - {provider}:{cfg.model}")

        # Choose call method based on provider
        for attempt in range(cfg.max_retries + 1):
            try:
                self.llm_logger.debug(
                    f"[REQ] Attempt {attempt + 1}/{cfg.max_retries + 1}"
                )

                # Prioritize using OpenAI official client (unless other provider explicitly specified)
                if provider in ["openai", "openai-official"] or provider not in [
                    "vllm",
                    "llama.cpp",
                    "none",
                    "openai-compatible",
                    ...
                    if result["status_code"] == 200:
                        ...
                        self._completion_tokens_used += usage.get(
                            "completion_tokens", 0
                        )
                        meta["usage"] = usage

                        self.llm_logger.info(
                            f"[OK] {role} success - Latency: {meta['latency_ms']}ms, Token usage: {usage}"
                        )

                        # Parse JSON content
                        if not content or not content.strip():
                            self.llm_logger.warning("[WARN] LLM returned empty content")
                        ...
                        if not content:
                            self.llm_logger.warning(
                                "[WARN] Content is empty after preprocessing, cannot parse JSON"
                            )
                            return {"raw_content": ""}, meta
                        ...
                        try:
                            ...
                            self.llm_logger.debug("[OK] Direct JSON parsing successful")
                            cache_write = (
                                cfg.cache_write_enabled
                                if cfg.cache_write_enabled is not None
                                else cfg.cache_enabled
                            )
                            ...
                            self.llm_logger.debug(
                                "[RAW] Try to extract <DECISION> tag content"
                            )
                            self.llm_logger.debug(
                                f"[RAW] Content length: {len(content)}"
                            )

                            # Try to extract JSON
                            extracted_json = self._extract_json_with_improved_logic(
                                content
                            )
                            if extracted_json:
                                self.llm_logger.debug("[OK] JSON extraction successful")
                                cache_write = (
                                    cfg.cache_write_enabled
                                    if cfg.cache_write_enabled is not None
                                    else cfg.cache_enabled
                            ...
                            self.llm_logger.warning(
                                "[WARN] JSON extraction failed, returning original content"
                            )
                            cache_write = (
                                cfg.cache_write_enabled
                                if cfg.cache_write_enabled is not None
                        ...
                        self.llm_logger.error(
                            f"[ERROR] Authentication error: {result.get('error', 'Unknown')}"
                        )
                        return None, {
                            **meta,
                            "reason": f"auth_error: {result.get('error', 'Unknown')}",
                        }
                        ...
                        self.llm_logger.warning(
                            f"[WARN] Rate limit - Wait {wait_time}s (Attempt {attempt + 1})"
                        )
                        ...
                        wait_time = cfg.backoff_factor**attempt
                        self.llm_logger.error(
                            f"[ERROR] API error {result['status_code']} - Attempt {attempt + 1}: {error_msg[:100]}"
                        )
                ...
                # Other providers use the original method
                    ...
                    if (
                        ...
                        or provider == "openai-compatible-no-auth"
                    ):
                ...
                if response and response.status_code == 200:
                    ...
                    self.llm_logger.info(
                        f"[OK] {role} success - Latency: {meta['latency_ms']}ms, Cache hit: False"
                    )
                    ...
                    try:
                        ...
                        self.llm_logger.debug(
                            f"📝 Response parsing - Content length: {len(content)}"
                        )
                        ...
                        if not content:
                            self.llm_logger.warning(
                                "⚠️ Content is empty after preprocessing, cannot parse JSON"
                            )
                            return {"raw_content": ""}, meta
                        ...
                        try:
                            ...
                            self.llm_logger.warning(
                                f"⚠️ JSON parsing failed, trying to extract: {str(e)[:50]}"
                            )
                            self.llm_logger.debug(
                                f"📄 Original content (first 200 chars): {content[:200]!r}"
                            )
                            ...
                            decision_match = re.search(
                                r"<DECISION>\s*(.*?)\s*</DECISION>", content, re.DOTALL
                            )
                            if decision_match:
                                ...
                                if (
                                    ...
                                    try:
                                        ...
                                        self.llm_logger.debug(
                                            f"🎯 DECISION tag JSON parsing successful"
                                        )
                                    except json.JSONDecodeError as e:
                                        self.llm_logger.debug(
                                            f"⚠️ DECISION tag JSON parsing failed: {str(e)[:50]}"
                                        )
                                        ...
                                        try:
                                            ...
                                            self.llm_logger.debug(
                                                f"🎯 DECISION tag JSON parsing successful after fixing"
                                            )
                                        except json.JSONDecodeError:
                                            self.llm_logger.debug(
                                                "⚠️ JSON fix attempt also failed"
                                            )
                            ...
                            self.llm_logger.warning(
                                f"⚠️ All JSON extraction methods failed, returning original content (length: {len(content)})"
                            )
                            cache_write = (
                                cfg.cache_write_enabled
                                if cfg.cache_write_enabled is not None
                    ...
                    self.llm_logger.warning(
                        f"⏱️ Rate limit - Wait {wait_time}s (Attempt {attempt + 1})"
                    )
                    if attempt < cfg.max_retries:
                        time.sleep(wait_time)
                        continue
                        ...
                        self.llm_logger.error(
                            f"❌ Rate limit exceeded maximum retry attempts"
                        )
                        return None, {**meta, "reason": "rate_limit_exceeded"}
                    ...
                    wait_time = backoff**attempt
                    self.llm_logger.error(
                        f"❌ HTTP error {response.status_code if response else 'None'} - Attempt {attempt + 1}: {error_msg[:100]}"
                    )
                ...
                wait_time = backoff**attempt
                self.llm_logger.error(
                    f"❌ Request exception {type(e).__name__} - Attempt {attempt + 1}: {str(e)[:100]}"
                )
                if attempt < cfg.max_retries:
                    time.sleep(wait_time)
                    continue
    ...
    def _call_openai_official(
        self, cfg: LLMConfig, system_prompt: str, user_prompt: str
    ) -> Dict[str, Any]:
    ...
    def _call_openai_compatible(
        self, cfg: LLMConfig, headers: Dict[str, str], body: Dict[str, Any]
    ) -> httpx.Response:
    ...
    def _call_vllm(
        ...
        """
        self.llm_logger.debug(
            f"🚀 Call vLLM API - Model: {body.get('model')}, URL: {cfg.base_url}"
        )
        ...
        try:
            ...
            if cfg.auth_required is False:
                ...
                self.llm_logger.debug(
                    "🔓 vLLM auth-free mode, remove Authorization header"
                )
            else:
            ...
            if response.status_code == 404:
                self.llm_logger.error(
                    f"❌ vLLM model not found: {body.get('model')} - Please check served-model-name configuration"
                )
            elif response.status_code == 422:
                self.llm_logger.error(
                    f"❌ vLLM parameter error - Please check temperature and other request parameters"
                )
            elif response.status_code == 200:
                self.llm_logger.debug(
                    f"✅ vLLM call successful - Status code: {response.status_code}"
                )
            else:
                self.llm_logger.warning(
                    f"⚠️ vLLM response abnormal - Status code: {response.status_code}"
                )
            ...
            self.llm_logger.error(
                f"❌ vLLM connection failed: {cfg.base_url} - {str(e)}"
            )
            self.llm_logger.error(
                "💡 Please ensure vLLM service is started: python -m vllm.entrypoints.openai.api_server --model <model_name> --port 8000"
            )
            raise
        except httpx.TimeoutException as e:
            self.llm_logger.error(
                f"❌ vLLM call timeout ({cfg.timeout_sec} seconds) - {str(e)}"
            )
            raise
        except Exception as e:
